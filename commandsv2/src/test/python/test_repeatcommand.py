...
def test_calls_methods_correctly(scheduler: commands2.CommandScheduler):
    ...
    assert command.is_finished.times_called == 0
    ...
    command.is_finished = lambda: False

    scheduler.run()
    assert command.initialize.times_called == 1
    assert command.execute.times_called == 1
    assert command.is_finished.times_called == 1
    assert command.end.times_called == 0
    ...
    assert command.is_finished.times_called == 2
    ...
    assert command.is_finished.times_called == 3
    assert command.end.times_called == 1
    ...
    assert command.is_finished.times_called == 4
