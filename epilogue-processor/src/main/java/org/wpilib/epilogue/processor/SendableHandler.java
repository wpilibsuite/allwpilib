...
public class SendableHandler extends ElementHandler {
  ...
  protected SendableHandler(ProcessingEnvironment processingEnv) {
    ...
    m_commandType =
        Optional.ofNullable(lookupTypeElement(processingEnv, "org.wpilib.command.Command"))
            .map(TypeElement::asType);
    m_subsystemType =
        Optional.ofNullable(lookupTypeElement(processingEnv, "org.wpilib.command.SubsystemBase"))
            .map(TypeElement::asType);
  }
