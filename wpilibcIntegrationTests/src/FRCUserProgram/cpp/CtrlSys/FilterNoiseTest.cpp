 ...
 */
class NoiseGenerator : public INode {
 public:
    ...
    switch (GetParam()) {
      case TEST_SINGLE_POLE_IIR: {
        m_filter = std::make_unique<LinearFilter>(
            kSinglePoleIIR, *m_noise, TestBench::kSinglePoleIIRTimeConstant,
            TestBench::kFilterStep);
        break;
      }

      case TEST_MOVAVG: {
        m_filter = std::make_unique<LinearFilter>(kMovingAverage, *m_noise,
...
TEST_P(FilterNoiseTest, NoiseReduce) {
  ...
  for (double t = 0; t < TestBench::kFilterTime; t += TestBench::kFilterStep) {
    ...
    filterError += std::abs(m_filter->GetOutput() - theoryData);
