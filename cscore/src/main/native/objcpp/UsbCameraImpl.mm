...
// @interface UsbCameraDelegate : NSObject
...
// - (id)init {
//     self = [super init];
//     m_inputImageHeader = cvCreateImageHeader(cvSize(1280, 960), IPL_DEPTH_8U,
//     4); m_outputImageHeader = cvCreateImageHeader(cvSize(1280, 960),
//     IPL_DEPTH_8U, 3); return self;
// }
...
//     auto image = source->AllocImage(cs::VideoMode::PixelFormat::kBGR, width,
//     height, currSize); m_outputImageHeader->imageData = image->data();
...
//         m_inputImageHeader->imageData = reinterpret_cast<char
//         *>(baseaddress); m_inputImageHeader->imageSize =
//         int(rowBytes*height);
...
// static constexpr const char* DeviceNotConnectedString =
// "DEVICENOTCONNECTEDATSTARTUP";
...
//     void UsbCameraImpl::SetProperty(int property, int value, CS_Status*
//     status) {}
...
//   {return true;} bool UsbCameraImpl::SetPixelFormat(VideoMode::PixelFormat
//   pixelFormat,
//                       CS_Status* status) {return true;}
//   bool UsbCameraImpl::SetResolution(int width, int height, CS_Status* status)
//   {return true;} bool UsbCameraImpl::SetFPS(int fps, CS_Status* status)
//   {return true;}
...
//   // Cache properties.  Immediately successful if properties are already
//   cached.
//   // If they are not, tries to connect to the camera to do so; returns false
//   and
...
//     auto queue = dispatch_queue_create("captureQueue",
//     DISPATCH_QUEUE_SERIAL);

//     [ objcData.output setSampleBufferDelegate: objcData.delegate
//     queue:queue];
...
//         }
//     } else {
//         NSString* idStr = [NSString stringWithCString:m_path.c_str()
...
// void UsbCameraImpl::PutFrameInternal(std::unique_ptr<Image> image,
// Frame::Time time) {
...
// CS_Source CreateUsbCameraDev(llvm::StringRef name, int dev, CS_Status*
// status) {
//     NSArray* captureDevices = [AVCaptureDevice
//     devicesWithMediaType:AVMediaTypeVideo];
