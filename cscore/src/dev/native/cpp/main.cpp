...
int main() {
  ...
  // CS_Source source = cs::CreateUsbCameraPath("Cam", "0x10000007ca313a",
  // &status);
  CS_Source source2 = cs::CreateUsbCameraPath(
      "Cam2", "EAB7A68F-EC2B-4487-AADF-D8A91C1CB782", &status);
  ...
  for (auto&& mode : modes) {
    std::cout << mode.width << "x" << mode.height << " at " << mode.fps
              << " FPS " << mode.pixelFormat << std::endl;
  }
  ...
  // cs::SetUsbCameraPath(source, "EAB7A68F-EC2B-4487-AADF-D8A91C1CB782",
  // &status);
