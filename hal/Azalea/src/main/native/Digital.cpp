...
static const uint32_t kExpectedLoopTiming = 260;
...
static MUTEX_ID digitalDIOSemaphore = NULL;
static MUTEX_ID digitalRelaySemaphore = NULL;
static MUTEX_ID digitalPwmSemaphore = NULL;
static MUTEX_ID digitalI2CSemaphore = NULL;
static tDIO* digitalModules[2] = {NULL, NULL};
static Resource *DIOChannels = NULL;
static Resource *DO_PWMGenerators[tDIO::kNumSystems] = {NULL};

bool digitalModulesInitialized = false;

/**
 * Initialize the digital modules.
 */
void initializeDigital(int32_t *status) {
  if (digitalModulesInitialized) return;
  ...
  for (unsigned int i = 0; i < (sizeof(digitalModules)/sizeof(digitalModules[0])); i++) {
    Resource::CreateResourceObject(&DIOChannels, tDIO::kNumSystems * kDigitalPins);
    Resource::CreateResourceObject(&DO_PWMGenerators[i], tDIO::kNumDO_PWMDutyCycleElements);
    digitalModules[i] = tDIO::create(i, status);
    
    // Make sure that the 9403 IONode has had a chance to initialize before continuing.
    while(digitalModules[i]->readLoopTiming(status) == 0) delayTicks(1);

    if (digitalModules[i]->readLoopTiming(status) != kExpectedLoopTiming) {
      // TODO: char err[128];
    ...
    double loopTime = digitalModules[i]->readLoopTiming(status)/(kSystemClockTicksPerMicrosecond*1e3);
    
    digitalModules[i]->writePWMConfig_Period((uint16_t) (kDefaultPwmPeriod/loopTime + .5), status);
    digitalModules[i]->writePWMConfig_MinHigh((uint16_t) ((kDefaultPwmCenter-kDefaultPwmStepsDown*loopTime)/loopTime + .5), status);
    ...
    for (uint32_t pwm_index = 1; pwm_index <= kPwmPins; pwm_index++) {
      ...
      digital_port->port.module = i + 1;
      digital_port->module = digitalModules[i];
    ...
    digitalModules[i]->writeSlowValue_RelayFwd(0, status);
    digitalModules[i]->writeSlowValue_RelayRev(0, status);
  }
  digitalModulesInitialized = true;
}
...
void* initializeDigitalPort(void* port_pointer, int32_t *status) {
  ...
  digital_port->module = digitalModules[digital_port->port.module-1];
...
uint8_t remapDigitalChannel(uint32_t pin, int32_t *status) {
  return 15 - pin; // TODO: Need channel validation
}
...
void setPWM(void* digital_port_pointer, unsigned short value, int32_t *status) {
  ...
  port->module->writePWMValue(port->port.pin - 1, value, status);
}
...
unsigned short getPWM(void* digital_port_pointer, int32_t *status) {
  DigitalPort* port = (DigitalPort*) digital_port_pointer;
  checkPWMChannel(port);
  return port->module->readPWMValue(port->port.pin - 1, status);
}
...
void setPWMPeriodScale(void* digital_port_pointer, uint32_t squelchMask, int32_t *status) {
  DigitalPort* port = (DigitalPort*) digital_port_pointer;
  checkPWMChannel(port);
  port->module->writePWMPeriodScale(port->port.pin - 1, squelchMask, status);
}
...
void* allocatePWMWithModule(uint8_t module, int32_t *status) {
  ...
  uint32_t* val = NULL;
  *val = DO_PWMGenerators[(module - 1)]->Allocate(buf);
  return val;
}
...
void freePWMWithModule(uint8_t module, void* pwmGenerator, int32_t *status) {
  ...
  if (id == ~0ul) return;
  DO_PWMGenerators[(module - 1)]->Free(id);
}
...
void setPWMRateWithModule(uint8_t module, double rate, int32_t *status) {
  ...
  uint8_t pwmPeriodPower = (uint8_t)(log(1.0 / (digitalModules[module-1]->readLoopTiming(status) * 0.25E-6 * rate))/log(2.0) + 0.5);
  digitalModules[module-1]->writeDO_PWMConfig_PeriodPower(pwmPeriodPower, status);
}
...
void setPWMDutyCycleWithModule(uint8_t module, void* pwmGenerator, double dutyCycle, int32_t *status) {
  ...
  if (rawDutyCycle > 255.5) rawDutyCycle = 255.5;
  {
    ...
    uint8_t pwmPeriodPower = digitalModules[module-1]->readDO_PWMConfig_PeriodPower(status);
    if (pwmPeriodPower < 4)
      {
    ...
    digitalModules[module-1]->writeDO_PWMDutyCycle(id, (uint8_t)rawDutyCycle, status);
  }
}
...
void setPWMOutputChannelWithModule(uint8_t module, void* pwmGenerator, uint32_t pin, int32_t *status) {
  ...
  switch(id) {
  case 0:
    digitalModules[module-1]->writeDO_PWMConfig_OutputSelect_0(remapDigitalChannel(pin - 1, status), status);
    break;
  case 1:
    digitalModules[module-1]->writeDO_PWMConfig_OutputSelect_1(remapDigitalChannel(pin - 1, status), status);
    break;
  case 2:
    digitalModules[module-1]->writeDO_PWMConfig_OutputSelect_2(remapDigitalChannel(pin - 1, status), status);
    break;
  case 3:
    digitalModules[module-1]->writeDO_PWMConfig_OutputSelect_3(remapDigitalChannel(pin - 1, status), status);
    break;
  }
}
...
void setRelayForward(void* digital_port_pointer, bool on, int32_t *status) {
  ...
  checkRelayChannel(port);
  {
    ...
    uint8_t forwardRelays = port->module->readSlowValue_RelayFwd(status);
    if (on)
      forwardRelays |= 1 << (port->port.pin - 1);
    else
      forwardRelays &= ~(1 << (port->port.pin - 1));
    port->module->writeSlowValue_RelayFwd(forwardRelays, status);
  }
}
...
void setRelayReverse(void* digital_port_pointer, bool on, int32_t *status) {
  ...
  {
    Synchronized sync(digitalRelaySemaphore);
    uint8_t reverseRelays = port->module->readSlowValue_RelayRev(status);
    if (on)
      reverseRelays |= 1 << (port->port.pin - 1);
    else
      reverseRelays &= ~(1 << (port->port.pin - 1));
    port->module->writeSlowValue_RelayRev(reverseRelays, status);
  }
}
...
bool allocateDIO(void* digital_port_pointer, bool input, int32_t *status) {
  ...
  if (DIOChannels->Allocate(kDigitalPins * (port->port.module - 1) + port->port.pin - 1, buf) == ~0ul) return false;
  {
    ...
    uint32_t outputEnable = port->module->readOutputEnable(status);
    ...
    port->module->writeOutputEnable(outputEnableValue, status);
  }
  return true;
}
...
void setDIO(void* digital_port_pointer, short value, int32_t *status) {
  ...
  {
    Synchronized sync(digitalDIOSemaphore);
    uint16_t currentDIO = port->module->readDO(status);
    if(value == 0) {
      currentDIO = currentDIO & ~(1 << remapDigitalChannel(port->port.pin - 1, status));
    } else if (value == 1) {
      currentDIO = currentDIO | (1 << remapDigitalChannel(port->port.pin - 1, status));
    } 
    port->module->writeDO(currentDIO, status);
  }
}
...
bool getDIO(void* digital_port_pointer, int32_t *status) {
  DigitalPort* port = (DigitalPort*) digital_port_pointer;
  uint32_t currentDIO = port->module->readDI(status);

  //Shift 00000001 over channel-1 places.
  //AND it against the currentDIO
  //if it == 0, then return false
  //else return true
  return ((currentDIO >> remapDigitalChannel(port->port.pin - 1, status)) & 1) != 0;
}
...
bool getDIODirection(void* digital_port_pointer, int32_t *status) {
  DigitalPort* port = (DigitalPort*) digital_port_pointer;
  uint32_t currentOutputEnable = port->module->readOutputEnable(status);
	
  //Shift 00000001 over port->port.pin-1 places.
  //AND it against the currentOutputEnable
  //if it == 0, then return false
  //else return true
  return ((currentOutputEnable >> remapDigitalChannel(port->port.pin - 1, status)) & 1) != 0;
}
...
void pulse(void* digital_port_pointer, double pulseLength, int32_t *status) {
  ...
  port->module->writePulseLength((uint8_t)(1.0e9 * pulseLength / (port->module->readLoopTiming(status) * 25)), status);
  port->module->writePulse(mask, status);
}
...
bool isPulsing(void* digital_port_pointer, int32_t *status) {
  DigitalPort* port = (DigitalPort*) digital_port_pointer;
  uint16_t mask = 1 << remapDigitalChannel(port->port.pin - 1, status);
  uint16_t pulseRegister = port->module->readPulse(status);
...
bool isAnyPulsingWithModule(uint8_t module, int32_t *status) {
  uint16_t pulseRegister = digitalModules[module-1]->readPulse(status);
...
uint16_t getLoopTiming(int32_t *status) {
  return getLoopTimingWithModule(0, status);
}
...
uint16_t getLoopTimingWithModule(uint8_t module, int32_t *status) {
  return digitalModules[module-1]->readLoopTiming(status);
}
