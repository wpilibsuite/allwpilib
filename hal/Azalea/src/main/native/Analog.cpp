...
static bool analogSampleRateSet[2] = {false, false};
static MUTEX_ID analogRegisterWindowSemaphore = NULL;
static tAI* analogModules[2] = {NULL, NULL};
static uint32_t analogNumChannelsToActivate[2] = {0, 0};
...
void initializeAnalog(int32_t *status) {
  ...
  analogRegisterWindowSemaphore = initializeMutex(SEM_Q_PRIORITY | SEM_DELETE_SAFE | SEM_INVERSION_SAFE);
...
void setAnalogSampleRate(double samplesPerSecond, int32_t *status) {
  setAnalogSampleRateWithModule(0, samplesPerSecond, status);
}
...
float getAnalogSampleRate(int32_t *status) {
  return getAnalogSampleRateWithModule(0, status);
}
