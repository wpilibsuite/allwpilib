...
void setOneShotDuration(HAL_SolenoidHandle solenoidPortHandle, int32_t durMS, int32_t *status) {
  auto port = solenoidHandles.Get(solenoidPortHandle);
  if (port == nullptr) {
    *status = HAL_HANDLE_ERROR;
    return;
  ...
  *status = PCM_modules[port->module]->SetOneShotDurationMs(port->channel, durMS);
}

void fireOneShot(HAL_SolenoidHandle solenoidPortHandle, int32_t *status) {
  auto port = solenoidHandles.Get(solenoidPortHandle);
  if (port == nullptr) {
    *status = HAL_HANDLE_ERROR;
    return;
