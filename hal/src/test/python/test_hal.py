...
def test_hal_simdevice():
    ...
    v = device.create_int("i", hal.SimDevice.Direction.INPUT, 1)
    assert v.get() == 1
...
def test_hal_simdevice_enum():
    ...
    names = ["one", "two", "three"]
    v = device.create_enum("e", hal.SimDevice.Direction.INPUT, names, 0)
...
def test_hal_simdevice_enum_double():
    ...
    names = ["one", "two", "three"]
    values = [0.0, 1.0, 2.0]
    v = device.create_enum_double("e", hal.SimDevice.Direction.INPUT, names, values, 0)
