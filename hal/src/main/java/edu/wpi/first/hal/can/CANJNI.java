...
/**
 * The CANJNI class provides Java Native Interface (JNI) methods for interfacing
 * with the
...
public class CANJNI extends JNIWrapper {
     ...
     * @param periodMs  The period in milliseconds at which to send the message, use
     *                  {@link #CAN_SEND_PERIOD_NO_REPEAT} for a single send.
     */
    ...
    /**
     * Opens a new CAN stream session for receiving CAN messages with specified
     * filters.
     *
     * @param messageID     The CAN messageID to match against. The bits of the
     *                      messageID are bitwise ANDed with the messageIDMask.
     * @param messageIDMask The CAN messageIDMask is a bit-wise mask of bits in the
     *                      messageID
     *                      to match against. This allows
     *                      matching against multiple frames. For example, providing
     *                      an messageID of 0x2050001 and a
     *                      mask of 0x1FFF003F would match all REV motor controller
     *                      frames for a device with CAN ID 1.
     *                      Providing a mask of 0x1FFFFFFF means that only the exact
     *                      messageID will be matched. Providing
     *                      a mask of 0 would match any frame of any type.
     * @param maxMessages   The maximum number of messages that can be buffered in
     *                      the session.
     ...
     * @param messages       An array to hold the CANStreamMessage objects (output
     *                       parameter).
     * @param messagesToRead The number of messages to read from the session.
     * @throws CANStreamOverflowException If the number of messages to read exceeds
     *                                    the capacity
     *                                    of the provided messages array.
