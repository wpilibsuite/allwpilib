...
int32_t HAL_GetJoystickType(int32_t joystickNum) {
    return (int32_t) mau::sharedMemory->Bagel_getJoyDescriptor(joystickNum).get_type();
}

char* HAL_GetJoystickName(int32_t joystickNum) {
    return mau::sharedMemory->Bagel_getJoyDescriptor(joystickNum).get_name();
}

void HAL_FreeJoystickName(char* name) { std::free(name); }

int32_t HAL_GetJoystickAxisType(int32_t joystickNum, int32_t axis) {
    return (int32_t)mau::sharedMemory->getJoystick(joystickNum).joyAxisType;
}
