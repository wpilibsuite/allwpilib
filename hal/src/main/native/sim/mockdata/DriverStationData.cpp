...
void DriverStationData::CallOpModesCallbacks(HAL_RobotMode mode) {
  switch (mode) {
    case HAL_kRobotMode_autonomous:
      m_autoOpModesCallbacks.Invoke(m_autoOpModes.size(), m_autoOpModes.data());
      break;
    case HAL_kRobotMode_teleop:
      m_teleopOpModesCallbacks.Invoke(m_teleopOpModes.size(),
                                      m_teleopOpModes.data());
      break;
    case HAL_kRobotMode_test:
      m_testOpModesCallbacks.Invoke(m_testOpModes.size(), m_testOpModes.data());
      break;
    default:
      break;
  }
...
#define DEFINE_CPPAPI_CALLBACKS(Upper, lower)                               \
  int32_t DriverStationData::Register##Upper##OpModesCallback(              \
    ...
    if (initialNotify) {                                                    \
      callback(Get##Upper##OpModesName(), param, m_##lower##OpModes.size(), \
               m_##lower##OpModes.data());                                  \
    }                                                                       \
    return uid;                                                             \
  }                                                                         \
                                                                            \
