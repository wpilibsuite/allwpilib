...
static void buffer_to_block(const unsigned char* buffer, uint32_t block[BLOCK_INTS])
{
...
static void finalize(uint32_t digest[], unsigned char* buffer, size_t& buf_size, uint64_t &transforms, llvm::raw_ostream& os)
{
    ...
    {
        uint32_t v = digest[i];
        os << LUT[(v >> 28) & 0xf] << LUT[(v >> 24) & 0xf]
           << LUT[(v >> 20) & 0xf] << LUT[(v >> 16) & 0xf]
           << LUT[(v >> 12) & 0xf] << LUT[(v >> 8) & 0xf]
