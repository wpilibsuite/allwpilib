 ...
 */
inline void ReportUsage(std::string_view resource, std::string_view data) {
  WPI_String resourceStr = wpi::util::make_string(resource);
  WPI_String dataStr = wpi::util::make_string(data);
  WPI_ReportUsage(&resourceStr, &dataStr);
}
 ...
 * @param data arbitrary associated data string
 */
void ReportUsage(std::string_view resource, int32_t instanceNumber,
                 std::string_view data);

/**
 * Sets the implementation used by ReportUsage().
 *
 * @param func function called by ReportUsage(); pass nullptr to restore the
 *             default no-op implementation
 */
void SetReportUsageImpl(WPI_ReportUsageImpl func);
