...
class DenseMapBase : public DebugEpochBase {
  ...
  using UsedT = wpi::util::densemap::detail::UsedT;
...
public:
  ...
  void clear() {
    ...
    std::memset(getUsed(), 0,
                wpi::util::densemap::detail::usedWords(getNumBuckets()) *
                    sizeof(UsedT));
    setNumEntries(0);
  }
  ...
  template <typename Predicate> bool remove_if(Predicate Pred) {
    ...
    for (unsigned I = 0; I != NumBuckets; ++I) {
      if (!wpi::util::densemap::detail::used(U, I))
        continue;
      if (Pred(B[I])) {
        B[I].getSecond().~ValueT();
        B[I].getFirst().~KeyT();
        wpi::util::densemap::detail::unsetUsed(U, I);
        decrementNumEntries();
        Removed = true;
      }
    }
...
protected:
  ...
  void destroyAll() {
    ...
    wpi::util::densemap::detail::forEachUsed(U, E, [&](unsigned I) {
      B[I].getSecond().~ValueT();
      B[I].getFirst().~KeyT();
    });
  }
  ...
  LLVM_ATTRIBUTE_NOINLINE void moveFrom(DerivedT &Other) {
    ...
    wpi::util::densemap::detail::forEachUsed(OtherU, E, [&](unsigned I) {
      ...
      while (wpi::util::densemap::detail::used(U, BucketNo))
      ...
      wpi::util::densemap::detail::setUsed(U, BucketNo);
  ...
  LLVM_ATTRIBUTE_NOINLINE void copyFrom(const DerivedT &other) {
    ...
    std::memcpy(U, OtherU,
                wpi::util::densemap::detail::usedWords(NumBuckets) * sizeof(UsedT));
    if constexpr (std::is_trivially_copyable_v<KeyT> &&
      ...
      wpi::util::densemap::detail::forEachUsed(U, NumBuckets, [&](unsigned I) {
...
private:
  ...
  LLVM_ATTRIBUTE_NOINLINE void eraseFromFilledBucket(BucketT *TheBucket,
    ...
    while (true) {
      ...
      if (!wpi::util::densemap::detail::used(U, J))
        break;
  ...
  BucketT *findBucketForInsertion(const LookupKeyT &Lookup,
    ...
    wpi::util::densemap::detail::setUsed(getUsed(), TheBucket - getBuckets());
  ...
  const BucketT *doFind(const LookupKeyT &Val) const {
    ...
    while (true) {
      ...
      if (LLVM_LIKELY(!wpi::util::densemap::detail::used(U, BucketNo)))
        return nullptr;
...
public:
  ...
  [[nodiscard]] size_t getMemorySize() const {
    return wpi::util::densemap::detail::allocBytes<BucketT>(getNumBuckets());
  }
};
...
public:
  ...
  DenseMap(wpi::util::from_range_t, const RangeT &Range)
...
private:
  ...
  void deallocateBuckets() {
    ...
    deallocate_buffer(Buckets,
                      wpi::util::densemap::detail::allocBytes<BucketT>(NumBuckets),
                      wpi::util::densemap::detail::allocAlign<BucketT>());
    Buckets = nullptr;
    Used = nullptr;
    NumBuckets = 0;
  }
  ...
  bool allocateBuckets(unsigned Num) {
    ...
    auto *Storage = static_cast<char *>(
        allocate_buffer(wpi::util::densemap::detail::allocBytes<BucketT>(NumBuckets),
                        wpi::util::densemap::detail::allocAlign<BucketT>()));
...
class SmallDenseMap
  ...
  static constexpr unsigned InlineUsedWords =
      wpi::util::densemap::detail::usedWords(InlineBuckets);
...
public:
  ...
  template <typename RangeT>
  SmallDenseMap(wpi::util::from_range_t, const RangeT &Range)
...
private:
  ...
  void swapImpl(SmallDenseMap &RHS) {
    ...
    if (Small && RHS.Small) {
      ...
      for (unsigned I = 0; I != InlineBuckets; ++I) {
        bool L = wpi::util::densemap::detail::used(LU, I);
        bool R = wpi::util::densemap::detail::used(RU, I);
        if (L && R) {
    ...
    LargeSide.Small = true;
    {
      ...
      for (unsigned I = 0; I != InlineBuckets; ++I)
        if (wpi::util::densemap::detail::used(SU, I))
  ...
  void deallocateBuckets() {
    ...
    deallocate_buffer(
        ...
        wpi::util::densemap::detail::allocBytes<BucketT>(storage.Large.NumBuckets),
        wpi::util::densemap::detail::allocAlign<BucketT>());
  ...
  bool allocateBuckets(unsigned Num) {
    ...
    auto *S = static_cast<char *>(
        allocate_buffer(wpi::util::densemap::detail::allocBytes<BucketT>(Num),
                        wpi::util::densemap::detail::allocAlign<BucketT>()));
...
public:
  ...
  static DenseMapIterator makeBegin(pointer Buckets, const UsedT *Used,
    ...
    auto R = maybeReverse(wpi::util::make_range(Buckets, Buckets + NumBuckets));
...
private:
  void AdvancePastEmptyBuckets() {
    if constexpr (shouldReverseIterate<KeyT>()) {
      while (Ptr != End && !wpi::util::densemap::detail::used(Used, &*Ptr - Buckets))
        ++Ptr;
    } else {
      ...
      const size_t NW = wpi::util::densemap::detail::usedWords(N);
      size_t W = I >> 5;
      ...
      Ptr = Buckets + ((W << 5) + std::countr_zero(Bits));
    }
  }
