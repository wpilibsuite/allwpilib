...
transformOptional(Optional &&O, Function &&F) {
  if (O) {
    return wpi::util::invoke(std::forward<Function>(F), *std::forward<Optional>(O));
  }
  return std::nullopt;
}
...
public:
  ...
  constexpr decltype(auto) operator()(CallArgsT &&...CallArgs) {
    if constexpr (BindFront)
      return wpi::util::invoke(FnStorage.get(), std::get<Indices>(BoundArgs)...,
                          std::forward<CallArgsT>(CallArgs)...);
    else
      return wpi::util::invoke(FnStorage.get(), std::forward<CallArgsT>(CallArgs)...,
