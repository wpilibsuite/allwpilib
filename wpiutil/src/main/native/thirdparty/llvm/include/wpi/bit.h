...
template <typename T> [[nodiscard]] int countr_zero(T Val) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return wpi::detail::TrailingZerosCounter<T, sizeof(T)>::count(Val);
}
...
template <typename T> [[nodiscard]] int countl_zero(T Val) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return wpi::detail::LeadingZerosCounter<T, sizeof(T)>::count(Val);
}
...
template <typename T> [[nodiscard]] int countl_one(T Value) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return wpi::countl_zero<T>(~Value);
}
...
template <typename T> [[nodiscard]] int countr_one(T Value) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return wpi::countr_zero<T>(~Value);
}
...
template <typename T> [[nodiscard]] int bit_width(T Value) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return std::numeric_limits<T>::digits - wpi::countl_zero(Value);
}
...
template <typename T> [[nodiscard]] T bit_floor(T Value) {
  ...
  if (!Value)
    return 0;
  return T(1) << (wpi::bit_width(Value) - 1);
}
...
template <typename T> [[nodiscard]] T bit_ceil(T Value) {
  ...
  if (Value < 2)
    return 1;
  return T(1) << wpi::bit_width<T>(Value - 1u);
}
