...
constexpr int bitsRequired(unsigned NumValues) {
  return NumValues == 0 ? 0 : std::bit_width(NumValues - 1);
}
...
// TODO: Switch to wpi::util::is_sorted when it becomes constexpr.
...
protected:
  ...
  friend struct ::wpi::util::CastInfo;
  template <typename> friend struct ::wpi::util::PointerLikeTypeTraits;
};
