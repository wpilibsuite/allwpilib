...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (ExePath.stem() == (L"AdvantageScope")) {
    ...
    return StartExeTool(AdvantageScopePath, Status);
    ...
    return StartExeTool(ElasticPath, Status);
  } else {
    return StartJavaTool(ExePath, Status);
  }
}

int StartJavaTool(std::filesystem::path& ExePath, DWORD& Status) {
  WCHAR ExePathRaw[1024];
...
int StartExeTool(std::filesystem::path& ExePath, DWORD& Status) {
  STARTUPINFOW StartupInfo;
  PROCESS_INFORMATION ProcessInfo;
