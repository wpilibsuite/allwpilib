...
/**
 * A version of {@link PS4Controller} with {@link AsyncTrigger} factories for command-based.
 *
 * @see PS4Controller
 */
@SuppressWarnings("MethodName")
public class AsyncCommandPS4Controller extends AsyncCommandGenericHID {
   ...
   */
  public AsyncCommandPS4Controller(int port) {
    super(port);
   ...
   *     AsyncScheduler#getDefaultButtonLoop() default scheduler button loop}.
   */
  public AsyncTrigger share() {
    return share(AsyncScheduler.getInstance().getDefaultButtonLoop());
  }
   ...
   *     loop.
   */
  public AsyncTrigger share(EventLoop loop) {
    return m_hid.share(loop).castTo(AsyncTrigger::new);
  }
