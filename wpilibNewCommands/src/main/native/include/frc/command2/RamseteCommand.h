...
class RamseteCommand : public CommandHelper<Command, RamseteCommand> {
 public:
   ...
   */
  RamseteCommand(Trajectory trajectory, std::function<Pose2d()> pose,
                 RamseteController controller,
                 SimpleMotorFeedforward<units::meters> feedforward,
                 DifferentialDriveKinematics kinematics,
                 std::function<DifferentialDriveWheelSpeeds()> wheelSpeeds,
 ...
 private:
  ...
  std::function<DifferentialDriveWheelSpeeds()> m_speeds;
  std::unique_ptr<PIDController> m_leftController;
  std::unique_ptr<PIDController> m_rightController;
