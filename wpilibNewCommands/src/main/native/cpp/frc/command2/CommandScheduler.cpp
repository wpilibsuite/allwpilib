...
#include "frc/command2/Subsystem.h"
...
class CommandScheduler::Impl {
 public:
  ...
  EventLoop defaultButtonLoop;
  // The set of currently-registered buttons that will be polled every
  // iteration.
  EventLoop* activeButtonLoop{&defaultButtonLoop};
...
CommandScheduler::CommandScheduler()
    : m_impl(new Impl), m_watchdog(TimedRobot::kDefaultPeriod, [] {
        std::puts("CommandScheduler loop time overrun.");
      }) {
  HAL_Report(HALUsageReporting::kResourceType_Command,
             HALUsageReporting::kCommand2_Scheduler);
  wpi::SendableRegistry::AddLW(this, "Scheduler");
  LiveWindow::SetEnabledCallback([this] {
    this->Disable();
    this->CancelAll();
  });
  LiveWindow::SetDisabledCallback([this] { this->Enable(); });
}

CommandScheduler::~CommandScheduler() {
  wpi::SendableRegistry::Remove(this);
  LiveWindow::SetEnabledCallback(nullptr);
  LiveWindow::SetDisabledCallback(nullptr);
...
EventLoop* CommandScheduler::GetActiveButtonLoop() const {
  return m_impl->activeButtonLoop;
}

void CommandScheduler::SetActiveButtonLoop(EventLoop* loop) {
  m_impl->activeButtonLoop = loop;
}

EventLoop* CommandScheduler::GetDefaultButtonLoop() const {
...
void CommandScheduler::Run() {
  ...
  EventLoop* loopCache = m_impl->activeButtonLoop;
