...
TEST(CommandConceptsTest, OwnedCommand) {
  static_assert(cmd::OwnedCommand<CommandPtr&&>);
  static_assert(cmd::OwnedCommand<std::unique_ptr<Command>>);
  static_assert(cmd::OwnedCommand<InstantCommand&&>);
  ...
  [[maybe_unused]] CommandPtr test = cmd::Sequence(
      std::move(uniquePtr), std::move(instantCommand), std::move(commandPtr));
}
