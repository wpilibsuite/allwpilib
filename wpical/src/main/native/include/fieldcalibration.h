...
namespace fieldcalibration
{
  ...
  std::tuple<Eigen::Matrix<double, 3, 3>, Eigen::Matrix<double, 8, 1>> load_camera_model(wpi::json json_data);
  std::map<int, wpi::json> load_ideal_map(std::string path);
  std::vector<Eigen::Vector3d> get_model_corners(double tag_size);
  Eigen::Matrix<double, 4, 4> get_tag_transform(std::map<int, wpi::json> &ideal_map, int tag_id);
