...
Worker::Worker(std::shared_ptr<Data> data, float squareWidth, float markerWidth,
  ...
  m_processingThread = std::thread([this, data] {
    while (!m_isDone) {
...
CameraCalibrator::CameraCalibrator(size_t numWorkers, double squareWidth,
  ...
  for (size_t i = 0; i < numWorkers; i++) {
    m_workers.emplace_back(std::make_unique<Worker>(
        m_state, m_squareWidth, markerWidth, boardWidth, boardHeight));
  }
}
...
void CameraCalibrator::ProcessVideo(std::string& videoPath) {
  ...
  m_processingThread =
      std::thread([this, cornerSize = m_cornerSize, squareWidth = m_squareWidth,
                   state = m_state, capture = std::move(cap)]() mutable {
        cv::Size frameShape{
        ...
        if (m_isFinished) {
          state->m_cameraModel = std::nullopt;
          return;
        }
        ...
        auto result =
            mrcal_main(allObservationBoards, allFramesRtToref, cornerSize,
                       squareWidth * 0.0254, frameShape, 1000);
        state->m_cameraModel = MrcalResultToCameraModel(*result);
        m_isFinished = true;
      });
  m_processingThread.detach();
}
