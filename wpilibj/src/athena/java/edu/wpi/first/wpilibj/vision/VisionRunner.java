 ...
 * and use the listener to take snapshots the pipeline's outputs.
 *
 * @see VisionPipeline
...
public class VisionRunner {
  ...
  public void runOnce() {
    ...
    if (frameTime == 0) {
      ...
      m_pipeline.setImageInput(m_image);
      m_pipeline.process();
      m_listener.copyPipelineOutputs();
    }
  }

  /**
   * A convenience method that calls {@link #runOnce()} in an infinite loop. This must
   * be run in a dedicated thread, and cannot be used in the main robot thread.
   *
