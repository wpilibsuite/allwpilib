 ...
 * robot drive class handles basic driving for a robot. Currently, 2 and 4 motor standard drive
 * trains are supported. In the future other drive types like swerve and meccanum might be
 * implemented. Motor channel numbers are passed supplied on creation of the class. Those are used
 * for either the drive function (intended for hand created drive code, such as autonomous) or with
...
public class RobotDrive implements MotorSafety {
   ...
   * call assumes Jaguars for controlling the motors.
   *
   ...
   * +1.0 values, where 0.0 represents stopped and not turning. curve < 0 will turn left and curve >
   * 0 will turn right.
   *
   * The algorithm for steering provides a constant turn radius for any normal speed range, both
   * forward and backward. Increasing m_sensitivity causes sharper turns for fixed values of curve.
   *
   * This function will most likely be used in an autonomous routine.
   *
   * @param outputMagnitude The speed setting for the outside wheel in a turn, forward or backwards,
   *                        +1 to -1.
   * @param curve           The rate of turn, constant for different forward speeds. Set curve < 0
   *                        for left turn or curve > 0 for right turn. Set curve = e^(-r/w) to get a
   *                        turn radius r for wheelbase w of your robot. Conversely, turn radius r =
   *                        -ln(curve)*w for a given value of curve and wheelbase w.
   */
   ...
   *
   * A method for driving with Mecanum wheeled robots. There are 4 wheels on the robot, arranged so
   * that the front and back wheels are toed in 45 degrees. When looking at the wheels from the top,
   * the roller axles should form an X across the robot.
   *
