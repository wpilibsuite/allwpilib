...
public final class DriverStationBackend {
  ...
  private static class MatchDataSender {
    ...
    private void sendMatchData() {
      ...
      var matchInfo = new MatchInfoData();
      DriverStationJNI.getMatchInfo(matchInfo);

      String currentEventName = matchInfo.eventName;
      String currentGameData;
      int currentMatchNumber = matchInfo.matchNumber;
      int currentReplayNumber = matchInfo.replayNumber;
      int currentMatchType = matchInfo.matchType;
  ...
  public static void silenceJoystickConnectionAlert(boolean silence) {
    ...
    try {
      ...
      if (silence && !RobotState.isFMSAttached()) {
  ...
  public static boolean isJoystickConnectionAlertSilenced() {
    m_joystickAlertMutex.lock();
    try {
      return !RobotState.isFMSAttached() && m_silenceJoystickAlerts;
    } finally {
      m_joystickAlertMutex.unlock();
    }
  }
  ...
  private static void reportJoystickResourceWarning(
    ...
    m_joystickAlertMutex.lock();
    try {
      if (!RobotState.isFMSAttached() && m_silenceJoystickAlerts) {
        return;
      }
