...
public interface LEDPattern {
  ...
  static LEDPattern steps(Map<? extends Number, Color> steps) {
    ...
    if (steps.isEmpty()) {
      // no colors specified
      DriverStationBackend.reportWarning("Creating LED steps with no colors!", false);
      return kOff;
    }

    if (steps.size() == 1 && steps.keySet().iterator().next().doubleValue() == 0) {
      // only one color specified, just show a static color
      DriverStationBackend.reportWarning("Creating LED steps with only one color!", false);
  ...
  static LEDPattern gradient(GradientType type, Color... colors) {
    ...
    if (colors.length == 0) {
      // Nothing to display
      DriverStationBackend.reportWarning("Creating a gradient with no colors!", false);
      return kOff;
    }

    if (colors.length == 1) {
      // No gradients with one color
      DriverStationBackend.reportWarning("Creating a gradient with only one color!", false);
