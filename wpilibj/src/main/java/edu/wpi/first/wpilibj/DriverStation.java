...
public final class DriverStation {
  ...
  public static boolean isTeleopEnabled() {
    m_cacheDataMutex.lock();
    try {
      return m_controlWord.getMode() == RobotMode.TELEOPERATED && m_controlWord.getEnabled();
    } finally {
      m_cacheDataMutex.unlock();
    }
  }
   ...
   * robot is enabled; use isEnabled() for that.
   *
   * @return Operating mode string; may return "", so callers should be prepared to handle that case
   */
  ...
  /**
   * Check to see if the current operating mode is a particular value. Note this does not mean the
