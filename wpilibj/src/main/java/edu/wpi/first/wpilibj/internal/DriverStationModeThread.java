...
public class DriverStationModeThread implements AutoCloseable {
  ...
  private final AtomicLong m_userOpMode = new AtomicLong();
  private final AtomicBoolean m_userEnabled = new AtomicBoolean();
  ...
  private void run() {
    ...
    while (m_keepAlive.get()) {
      ...
      DriverStationJNI.observeUserProgramOpMode(m_userOpMode.get(), m_userEnabled.get());
    }
   ...
   *
   * @param id opmode UID
   * @param enabled If true, running enabled code; if false, running disabled code
   */
  public void inOpMode(long id, boolean enabled) {
    m_userOpMode.set(id);
    m_userEnabled.set(enabled);
  }
