...
public class ADXL345_I2C implements TelemetryLoggable, AutoCloseable {
  ...
  public record AllAxes(double x, double y, double z) implements StructSerializable {
    public class AllAxesStruct implements Struct<AllAxes> {
      @Override
  ...
  public AllAxes getAccelerations() {
    ...
    return new AllAxes(rawData.getShort(0) * kGsPerLSB, rawData.getShort(2) * kGsPerLSB, rawData.getShort(4) * kGsPerLSB);
  }

  @Override
  public void toTelemetry(TelemetryTable table, boolean first) {
    if (first) {
      table.log(".type", "3AxisAccelerometer");
    }
