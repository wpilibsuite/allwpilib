...
class Mechanism2dTest {
  ...
  @Test
  void testCanvas() {
    var mechanism = new Mechanism2d(5, 10);
    Telemetry.log("mechanism", mechanism);
    assertArrayEquals(new double[] {5, 10}, m_mock.getLastValue("/mechanism/dims", double[].class));
    assertEquals(
        "#000020", m_mock.getLastValue("/mechanism/backgroundColor", LogStringValue.class).value());

    mechanism.setBackgroundColor(new Color8Bit(255, 255, 255));
    Telemetry.log("mechanism", mechanism);
    assertEquals(
        "#FFFFFF", m_mock.getLastValue("/mechanism/backgroundColor", LogStringValue.class).value());
  }
  ...
  void testLigament() {
    ...
    Telemetry.log("mechanism", mechanism);
    assertEquals(
        ligament.getAngle(), m_mock.getLastValue("/mechanism/root/ligament/angle", Double.class));
    assertEquals(
        ligament.getColor().toHexString(),
        m_mock.getLastValue("/mechanism/root/ligament/color", LogStringValue.class).value());
    assertEquals(
        ligament.getLength(), m_mock.getLastValue("/mechanism/root/ligament/length", Double.class));
    assertEquals(
