...
public final class TelemetryTable {
  ...
  public TelemetryTable getTable(String name) {
    return m_tablesMap.computeIfAbsent(
        name,
        k ->
            m_backend != null
                ? new TelemetryTable(m_path + k + "/", m_backend)
                : TelemetryRegistry.getTable(m_path + k + "/"));
  }
  ...
  private TelemetryEntry getEntry(String name) {
    ...
    return m_entriesMap.computeIfAbsent(
        name,
        k ->
            m_backend != null
                ? m_backend.getEntry(m_path + k)
                : TelemetryRegistry.getEntry(m_path + k));
  }
