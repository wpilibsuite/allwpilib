...
namespace wpi::telemetry::detail {
...
inline std::string_view NormalizeTableName(std::string_view path,
                                           std::string& buf) {
  // common case is a well formatted name, so check first
  if (wpi::util::starts_with(path, '/') && wpi::util::ends_with(path, '/') &&
      !wpi::util::contains(path, "//")) {
    return path;
  }
  std::string_view normalized = NormalizeName(path, buf);
  if (!wpi::util::ends_with(normalized, '/')) {
    if (buf.empty()) {
      buf = normalized;
    }
    buf.push_back('/');
