...
class TelemetryRegistry final {
 ...
 public:
  ...
  /**
   * Get a telemetry entry for a particular name. The returned handle keeps the
   * entry's backend alive while the entry is in use.
   *
   * @param path full name
   * @return telemetry entry
   */
  static std::shared_ptr<TelemetryEntry> GetEntry(std::string_view path);
 ...
 private:
  ...
  static void RecordKeepDuplicates(std::string_view path);

  static void RecordProperty(std::string_view path, std::string_view key,
                             std::string_view value);

  static void ApplyEntryMetadata(std::string_view path, TelemetryEntry& entry);

  static EntryHandle GetEntryHandle(std::string_view path);
