...
void glass::DisplayExponentialPIDController(ExponentialPIDControllerModel* m) {
  ...
  if (m->Exists()) {
    ...
    if (auto s = m->GetAData()) {
      double value = s->GetValue();
      createTuningParameter("A", &value, [=](auto v) { m->SetA(v); });
    }
    if (auto s = m->GetBData()) {
      double value = s->GetValue();
      createTuningParameter("B", &value, [=](auto v) { m->SetB(v); });
    }
