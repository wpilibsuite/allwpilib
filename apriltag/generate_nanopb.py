...
def generate_nanopb(
    nanopb: Path, output_directory: Path, proto_dir: Path, proto_path: Path
):
    ...
    cpp_files = (output_directory).glob("*")
    for cpp_file in cpp_files:
        with cpp_file.open(encoding="utf-8") as f:
            content = f.read()
...
def main():
    ...
    parser.add_argument(
        ...
        default=dirname / "src/generated/main/native/cpp/apriltag/protobuf",
        type=Path,
    )
    parser.add_argument(
        "--proto_directory",
    ...
    generate_nanopb(
        args.nanopb, args.output_directory, args.proto_directory, args.proto_path
    )
