...
wpi::util::expected<std::string, size_t> UnescapeURI(std::string_view str) {
  ...
  for (auto i = str.begin(), end = str.end(); i != end; ++i) {
    ...
    if (i + 2 >= end) {
      return wpi::util::unexpected{i - str.begin()};
    }
...
HttpQueryMap::HttpQueryMap(std::string_view query) {
  wpi::util::split(query, '&', 100, false, [&](auto elem) {
    ...
    if (auto name = wpi::net::UnescapeURI(nameEsc)) {
...
std::optional<std::string> HttpQueryMap::Get(std::string_view name) const {
  ...
  return wpi::net::UnescapeURI(it->second).value_or(std::string{});
}
...
HttpPath::HttpPath(std::string_view path) {
  ...
  wpi::util::split(path, '/', 100, false, [&](auto elem) {
    auto val = wpi::net::UnescapeURI(elem);
    if (!val) {
      m_pathEnds.clear();
      return;
    }
...
bool HttpPath::startswith(size_t start,
  ...
  for (auto m : match) {
    auto val = wpi::util::slice(m_pathBuf, first ? 0 : *(p - 1), *p);
    if (val != m) {
      return false;
    }
    first = false;
...
std::string_view HttpPath::operator[](size_t n) const {
  return wpi::util::slice(m_pathBuf, n == 0 ? 0 : m_pathEnds[n - 1],
                          m_pathEnds[n]);
}
...
bool ParseHttpHeaders(wpi::util::raw_istream& is, std::string* contentType,
  ...
  for (;;) {
    auto line = wpi::util::rtrim(is.getline(1024));
    if (is.has_error()) {
      return false;
    }
    if (line.empty()) {
...
void HttpRequest::SetAuth(const HttpLocation& loc) {
  if (!loc.user.empty()) {
    auth =
        wpi::util::Base64Encode(fmt::format("{}:{}", loc.user, loc.password));
  }
}
