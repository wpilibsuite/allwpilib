...
public class MulticastServiceAnnouncerTest {
    ...
    public void emptyText() throws InterruptedException, UnknownHostException {
        final String serviceName = "TestServiceNoText";
        ...
        try (MulticastServiceAnnouncer announcer =
             ...
             MulticastServiceResolver resolver =
                    new MulticastServiceResolver(serviceType)) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                for (ServiceData it : data) {
                    ...
                    System.out.printf("service %s at host %s ipv4 %s%n",
                            it.getServiceName(), it.getHostName(), ipString);
                    allData.add(it);
                }
            ...
            assertFalse(allData.isEmpty(),
                    "Expected at least one service entry, but got none");
