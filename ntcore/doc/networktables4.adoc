...
|`cached`|boolean|Cached Flag|If false, the server will not store the value of the topic.  This means that clients will not receive a value on subscription (only for value changes after subscription), and the persistent property will not cause the last set value to be saved to persistent storage.
|===
