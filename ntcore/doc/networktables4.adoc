...
|Boolean
|All Changes Flag
|If true, the server should send all value changes over the wire regardless of the `immediate` and `periodic` settings.  If false, only the most recent value is sent (same as NT 3.0 behavior).  If not specified, defaults to false.
|===
...
Sent from a client to the server to change properties (see <<properties>>) for a given topic.  The server will respond with an updated <<msg-announce>> (if the topic is published).  This message shall be ignored by the server if the topic is not published.
