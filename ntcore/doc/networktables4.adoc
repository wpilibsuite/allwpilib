...
== Motivation for Version 4.2

NetworkTables 4.2 link:time-sync-protocol.md[improves time synchronization] via a custom UDP protocol on a dedicated time-sync event loop.
...
==== Time Synchronization, clients < 4.2
...
link:time-sync-protocol.adoc[Time Sync Protocol] documentation describes requirements for clients and servers.
