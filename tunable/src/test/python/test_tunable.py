...
def backend():
    ...
    try:
        ...
        tunable.TunableRegistry.set_report_warning(None)
        tunable.TunableRegistry.reset()
...
def test_report_warning_allows_reentry(backend):
    ...
    def report_warning(msg: str) -> None:
        ...
        if msg == "outer warning":
            tunable.TunableRegistry.report_warning("nested warning")

    tunable.TunableRegistry.set_report_warning(report_warning)

    tunable.TunableRegistry.report_warning("outer warning")
...
def run_waiting_call(waiting_call):
    ...
    def holder():
        ...
        tunable.TunableRegistry.with_update_mutex(hold)
...
run_waiting_call(lambda: tunable.TunableRegistry.with_update_mutex(lambda: None))
run_waiting_call(tunable.TunableRegistry.update)
"""
...
def test_get_backend_normalizes_path(backend):
    ...
    assert tunable.TunableRegistry.get_backend("child/value") is child_backend
    assert tunable.TunableRegistry.get_backend("//child//value") is child_backend

    tunable.TunableRegistry.reset()
    child_only_backend = tunable.MockTunableBackend()
    tunable.TunableRegistry.register_backend("/child", child_only_backend)

    assert tunable.TunableRegistry.get_backend("child/value") is child_only_backend
...
def test_remove_complex_tunable_removes_members(backend):
    class RemovedComplex(tunable.ComplexTunable):
        def __init__(self) -> None:
            self.value = tunable.Tunable(1)
            self.update_count = 0
...
def test_table_remove_releases_complex_tunable(backend):
    class RemovedComplex(tunable.ComplexTunable):
        def __init__(self) -> None:
            self.value = tunable.Tunable(1)
...
def test_complex_table_remove_releases_published_value_child(backend):
    ...
    class RemovingComplex(tunable.ComplexTunable):
        def __init__(self) -> None:
            self.child = ChildValue()
            self.table: tunable.TunableTable | None = None
...
def test_complex_table_remove_releases_nested_complex_child(backend):
    ...
    class NestedComplex(tunable.ComplexTunable):
        def __init__(self) -> None:
...
def test_register_backend_migrates_complex_tunable(backend):
    class MigratedComplex(tunable.ComplexTunable):
        def __init__(self) -> None:
            self.value = tunable.Tunable(2)
            self.update_count = 0
...
def test_migrated_complex_publish_value_refreshes_once(backend):
    ...
    child_backend = tunable.MockTunableBackend()
    tunable.TunableRegistry.register_backend("/child", child_backend)
    complex_backend = tunable.MockTunableBackend()
    tunable.TunableRegistry.register_backend("/child/complex", complex_backend)
...
def test_more_specific_child_backend_keeps_migrated_complex_child(backend):
    ...
    leaf_backend = tunable.MockTunableBackend()
...
def test_complex_tunable_publish_descriptor_is_reused_for_initial_publish(backend):
    class DescriptorComplex:
        ...
        @property
        def publish_tunable(self):
            self.lookup_count += 1

            def publish(table: tunable.TunableTable) -> None:
...
def test_complex_tunable_direct_struct_publish_value(backend):
    class DirectStructComplex(tunable.ComplexTunable):
        def __init__(self) -> None:
...
def test_complex_tunable_direct_publish_value_refreshes_before_update(backend):
    class DirectGetterComplex(tunable.ComplexTunable):
        def __init__(self) -> None:
            self.value = 1
...
def test_complex_tunable_getter_can_mutate_top_level_storage_during_refresh(
    ...
    class EmptyComplex(tunable.ComplexTunable):
        def publish_tunable(self, table: tunable.TunableTable) -> None:
            pass

    class MutatingComplex(tunable.ComplexTunable):
        def __init__(self) -> None:
            self.armed = False
            self.calls = 0
            self.value = 1
...
def test_complex_tunable_wrapped_struct_member(backend):
    class WrappedStructComplex(tunable.ComplexTunable):
        def __init__(self) -> None:
            self.point = tunable.Tunable(TunablePoint(1, 2))
