...
void HALSimDSNT::Initialize() {
  rootTable = nt::NetworkTableInstance::GetDefault().GetTable("sim")->GetSubTable("DS_CONTROL");  // Not to be confused with sim::DriverStation from HALSim LowFi

  // LOOP TIMING //
  auto timinghz = rootTable->GetEntry("timing_hz");
  timinghz.ForceSetDouble(50);
  timinghz.AddListener([this](const nt::EntryNotification &ev) -> void {
  ...
  mtele.AddListener([this](const nt::EntryNotification &ev) -> void {
    this->HandleModePress(HALSimDSNT_Mode::teleop, ev.value->GetBoolean());
  }, NT_NotifyKind::NT_NOTIFY_UPDATE);

  mauto.AddListener([this](const nt::EntryNotification &ev) -> void {
    this->HandleModePress(HALSimDSNT_Mode::auton, ev.value->GetBoolean());
  }, NT_NotifyKind::NT_NOTIFY_UPDATE);

  mtest.AddListener([this](const nt::EntryNotification &ev) -> void {
    this->HandleModePress(HALSimDSNT_Mode::test, ev.value->GetBoolean());
  }, NT_NotifyKind::NT_NOTIFY_UPDATE);

  enabled.AddListener([this](const nt::EntryNotification &ev) -> void {
  ...
  estop.AddListener([this](const nt::EntryNotification &ev) -> void {
    std::lock_guard<wpi::mutex> lock(modeMutex);
  ...
  allianceStation.AddListener([this](const nt::EntryNotification &ev) -> void {
  ...
  allianceColorRed.AddListener([this](const nt::EntryNotification &ev) -> void {
...
void HALSimDSNT::UpdateModeButtons() {
  modeTable->GetEntry("teleop?").ForceSetBoolean(currentMode == HALSimDSNT_Mode::teleop);
  modeTable->GetEntry("auto?").ForceSetBoolean(currentMode == HALSimDSNT_Mode::auton);
  modeTable->GetEntry("test?").ForceSetBoolean(currentMode == HALSimDSNT_Mode::test);
...
void HALSimDSNT::DoAllianceUpdate() {
  HALSIM_SetDriverStationAllianceStationId(static_cast<HAL_AllianceStationID>((isAllianceRed ? HAL_AllianceStationID_kRed1 : HAL_AllianceStationID_kBlue1) + (static_cast<int32_t>(allianceStation) - 1)));
}

void HALSimDSNT::LoopFunc() {
  while (running) {
    double dt = 1000 / timingHz;
    std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<long long>(dt)));
    if (isEnabled) {
