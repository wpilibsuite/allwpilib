...
void HALSimWSProviderCompressor::RegisterCallbacks() {
    ...
    m_closedLoopCbKey = REGISTER_COMPRESSOR_NOPREFIX(ClosedLoopEnabled, "<>closed_loop", bool, boolean);
    m_pressureSwitchCbKey = REGISTER_COMPRESSOR_NOPREFIX(PressureSwitch, "<pressure_switch", bool, boolean);
    m_currentCbKey = REGISTER_COMPRESSOR(Current, "<current", double, double);
}

void HALSimWSProviderCompressor::CancelCallbacks() {
    DoCancelCallbacks();
}
...
void HALSimWSProviderSolenoid::Initialize(WSRegisterFunc webRegisterFunc) {
    CreateProviders<HALSimWSProviderSolenoid>("Solenoid", HAL_GetNumSolenoidChannels(), webRegisterFunc);
}
