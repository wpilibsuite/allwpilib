...
public final class Units {
  ...
  public static final class UnitBuilder<U extends Unit<U>> {
    ...
    /**
     * Sets the unit conversions based on a simple offset. The new unit will have its values equal
     * to (base value - offset).
     *
    ...
    private class MappingBuilder {
      ...
      /**
       * Maps a value {@code value} in the range {@code [inMin..inMax]} to an output in the range
       * {@code [outMin..outMax]}. Inputs outside the bounds will be mapped correspondingly to
       * outputs outside the output bounds. Inputs equal to {@code inMin} will be mapped to {@code
       * outMin}, and inputs equal to {@code inMax} will similarly be mapped to {@code outMax}.
       *
      ...
      public UnitBuilder<U> toOutputRange(double minOutput, double maxOutput) {
        UnitBuilder.this.m_fromBase =
            x -> mapValue(x, m_minInput, m_maxInput, minOutput, maxOutput);
     ...
     * being 1/100th of the base unit (meters). The fraction value is specified as the denominator
     * of the fraction, so a centimeter definition would use {@code splitInto(100)} instead of
     * {@code splitInto(1/100.0)}.
     *
    ...
    /**
     * Helper for defining units that are a scalar multiple of the base unit, such as kilometers
     * being 1000x of the base unit (meters).
     *
    ...
    public U make() {
      ...
      try {
        Constructor<? extends U> ctor =
            m_base.m_baseType.getDeclaredConstructor(
