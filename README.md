...
`testDesktopCpp` and `testDesktopJava` tasks also exist for the projects `wpiutil`, `ntcore`, `cscore`, `hal` `commands` and `cameraserver`. These can be ran with `./gradlew :projectName:task`.
