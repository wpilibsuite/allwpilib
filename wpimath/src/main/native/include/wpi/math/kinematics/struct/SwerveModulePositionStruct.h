// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

#pragma once

#include <wpi/SymbolExports.h>
#include <wpi/struct/Struct.h>

#include "wpi/math/kinematics/SwerveModulePosition.h"

template <>
struct WPILIB_DLLEXPORT wpi::Struct<wpi::math::SwerveModulePosition> {
  static constexpr std::string_view GetTypeName() {
    return "SwerveModulePosition";
  }
  static constexpr size_t GetSize() {
    return 8 + wpi::GetStructSize<wpi::math::Rotation2d>();
  }
  static constexpr std::string_view GetSchema() {
    return "double distance;Rotation2d angle";
  }

  static wpi::math::SwerveModulePosition Unpack(std::span<const uint8_t> data);
  static void Pack(std::span<uint8_t> data,
                   const wpi::math::SwerveModulePosition& value);
  static void ForEachNested(
      std::invocable<std::string_view, std::string_view> auto fn) {
    wpi::ForEachStructSchema<wpi::math::Rotation2d>(fn);
  }
};

static_assert(wpi::StructSerializable<wpi::math::SwerveModulePosition>);
static_assert(wpi::HasNestedStruct<wpi::math::SwerveModulePosition>);
