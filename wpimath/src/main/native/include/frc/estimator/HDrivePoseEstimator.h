...
#include "frc/kinematics/HDriveWheelSpeeds.h"
#include "units/time.h"

namespace frc {
/**
 * This class wraps H Drive Odometry to fuse latency-compensated
 * vision measurements with H drive encoder measurements. It will
 * correct for noisy vision measurements and encoder drift. It is intended to be
 * an easy drop-in for HDriveOdometry. In fact, if you never call
 * AddVisionMeasurement(), and only call Update(), this will behave exactly the
 * same as HDriveOdometry.
 *
 ...
 */
class WPILIB_DLLEXPORT HDrivePoseEstimator {
 public:
  /**
   * Constructs a HDrivePoseEstimator with default standard
   ...
   * @param leftDistance The distance traveled by the left encoder.
   * @param rightDistance The distance traveled by the right encoder.
   * @param lateralDistance The distance traveled by the lateral
   * @param initialPose The estimated initial pose.
 ...
 private:
  struct InterpolationRecord {
     ...
     */
    InterpolationRecord Interpolate(HDriveKinematics& kinematics,
  ...
  HDriveKinematics& m_kinematics;
  HDriveOdometry m_odometry;
