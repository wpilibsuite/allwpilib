...
template <int Rows, int Cols, int Options, int MaxRows, int MaxCols>
  requires(Cols != 1)
struct wpi::Struct<wpimath::Matrixd<Rows, Cols, Options, MaxRows, MaxCols>> {
  ...
  static void Pack(
      ...
      const wpimath::Matrixd<Rows, Cols, Options, MaxRows, MaxCols>& value) {
    constexpr size_t kDataOff = 0;
...
static_assert(wpi::StructSerializable<wpimath::Matrixd<1, 2>>);
static_assert(wpi::StructSerializable<wpimath::Matrixd<3, 3>>);
