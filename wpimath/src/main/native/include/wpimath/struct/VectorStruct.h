...
struct wpi::Struct<wpimath::Matrixd<Size, 1, Options, MaxRows, MaxCols>> {
  ...
  static wpimath::Matrixd<Size, 1, Options, MaxRows, MaxCols> Unpack(
      std::span<const uint8_t> data) {
    constexpr size_t kDataOff = 0;
    wpi::array<double, Size> vec_data =
        wpi::UnpackStructArray<double, kDataOff, Size>(data);
    wpimath::Matrixd<Size, 1, Options, MaxRows, MaxCols> vec;
  ...
  static void Pack(
      ...
      const wpimath::Matrixd<Size, 1, Options, MaxRows, MaxCols>& value) {
    constexpr size_t kDataOff = 0;
...
static_assert(wpi::StructSerializable<wpimath::Vectord<1>>);
static_assert(wpi::StructSerializable<wpimath::Vectord<3>>);
