...
template <int Rows, int Cols, int Options, int MaxRows, int MaxCols>
  requires(Cols != 1)
struct wpi::Protobuf<wpimath::Matrixd<Rows, Cols, Options, MaxRows, MaxCols>> {
  using MessageStruct = wpi_proto_ProtobufMatrix;
  using InputStream = wpi::ProtoInputStream<
      wpimath::Matrixd<Rows, Cols, Options, MaxRows, MaxCols>>;
  using OutputStream = wpi::ProtoOutputStream<
      wpimath::Matrixd<Rows, Cols, Options, MaxRows, MaxCols>>;

  static std::optional<wpimath::Matrixd<Rows, Cols, Options, MaxRows, MaxCols>>
