  ...
  (JNIEnv* env, jclass)
{
  ...
  problem->add_persistent_callback([](const slp::IterationInfo& info) -> bool {
    ...
    // Find IterationInfo class
    static wpi::java::JClass infoCls{
        callbackEnv, "edu/wpi/first/math/optimization/solver/IterationInfo"};
    if (!infoCls) {
      return true;
    }

    // Find Problem.runCallbacks() method
    static jmethodID runCallbacksMethodId = callbackEnv->GetMethodID(
        cls, "runCallbacks",
        "(Ledu/wpi/first/math/optimization/solver/IterationInfo;)Z");
    if (!runCallbacksMethodId) {
      return true;
    }

    // Find IterationInfo constructor
    static jmethodID infoCtor =
        callbackEnv->GetMethodID(infoCls, "<init>",
                                 "("
                                 "I"
    ...
    // Convert C++ IterationInfo into Java IterationInfo
    jobject infoObj =
        callbackEnv->NewObject(infoCls, infoCtor, info.iteration,
                               frc::EigenToEJML(callbackEnv, info.x),
                               frc::EigenToEJML(callbackEnv, info.g),
                               frc::EigenToEJML(callbackEnv, info.H),
                               frc::EigenToEJML(callbackEnv, info.A_e),
                               frc::EigenToEJML(callbackEnv, info.A_i));

    // Call Problem.runCallbacks()
    return callbackEnv->CallBooleanMethod(callbackObj, runCallbacksMethodId,
                                          infoObj);
  });
