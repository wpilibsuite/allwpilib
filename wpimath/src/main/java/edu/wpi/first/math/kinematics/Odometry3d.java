...
public class Odometry3d<T> {
  ...
  public Odometry3d(
    ...
    // When applied extrinsically (rhs of addition), m_gyroOffset cancels the current gyroAngle and
    // then rotates to m_poseMeters.getRotation()
    m_gyroOffset = gyroAngle.unaryMinus().plus(m_poseMeters.getRotation());
