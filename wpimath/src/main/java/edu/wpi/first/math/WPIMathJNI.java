...
public final class WPIMathJNI {
   ...
   *
   * <p>
   * Only use this function if you're sure the preconditions are met. Solves the
   * discrete
   ...
   * is the feedback
   * control law uₖ that minimizes the following cost function subject to xₖ₊₁ =
   * Axₖ + Buₖ:
   *
   * <pre>
   *     ∞
   ...
   * @throws IllegalArgumentException if the (A, C) pair where Q = CᵀC isn't
   *                                  detectable.
   */
   ...
   * @param inputs Number of inputs in B matrix.
   * @param S      Array storage for DARE solution.
   * @throws IllegalArgumentException if Q − NR⁻¹Nᵀ isn't symmetric positive
   *                                  semidefinite.
   * @throws IllegalArgumentException if R isn't symmetric positive definite.
   * @throws IllegalArgumentException if the (A − BR⁻¹Nᵀ, B) pair isn't
   *                                  stabilizable.
   * @throws IllegalArgumentException if the (A, C) pair where Q = CᵀC isn't
   *                                  detectable.
   */
  ...
  /**
   * Performs an inplace rank one update (or downdate) of an upper triangular
  ...
  /**
   * Solves the least-squares problem Ax=B using a QR decomposition with full
   * pivoting.
   *
   ...
   * @param dst   Array to store solution in. If A is m-n and B is m-p, dst is
   *              n-p.
   */
   ...
   * any, have
   * absolute values less than one, where an eigenvalue is uncontrollable if
   * rank(lambda * I - A, B)
