...
/**
 * Represents a trajectory consisting of a list of {@link TrajectorySample}s,
  ...
  public Trajectory(List<TrajectorySample> samples) {
    this.samples =
        new InterpolatingTreeMap<>(
                (start, end, q) -> MathUtil.inverseInterpolate(start.in(Seconds), end.in(Seconds), q.in(Seconds)),
                TrajectorySample::interpolate
        );
    ...
    this.duration = samples.get(samples.size() - 1).timestamp;
  }
