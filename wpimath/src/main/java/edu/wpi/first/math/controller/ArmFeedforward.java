...
import static edu.wpi.first.math.autodiff.Variable.sign;

import edu.wpi.first.math.autodiff.Gradient;
import edu.wpi.first.math.autodiff.Hessian;
import edu.wpi.first.math.autodiff.NumericalIntegration;
...
public class ArmFeedforward implements ProtobufSerializable, StructSerializable {
  ...
  public double calculate(
    ...
    m_dt = dt;
    return calculateWithVelocities(currentAngle, currentVelocity, nextVelocity);
  }
            ...
            (VariableMatrix x, VariableMatrix u) -> {
              var c =
                  new VariableMatrix(
                      new Variable[][] {
                        {new Variable(0.0)},
                        {sign(x.get(1)).times(-ks / ka).plus(cos(x.get(0)).times(-kg / ka))}
                      });
