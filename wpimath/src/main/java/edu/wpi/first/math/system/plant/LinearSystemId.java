...
public final class LinearSystemId {
  ...
  /**
   * Create the lateral plant of a state-space model of a H-drive drivetrain. In this model, the
   * states are [left velocity, right velocity, lateral velocity]ᵀ, inputs are [left voltage, right
   * voltage, lateral voltage]ᵀ, and outputs are [left velocity, right velocity, lateral velocity]ᵀ.
   *
   * @param motor The motor (or gearbox) driving the drivetrain.
   * @param massKg The mass of the robot in kilograms.
   * @param rMeters The radius of the wheels in meters.
   * @param offsetFromCenterOfGravityMeters The distance of the lateral movement wheel from the
   *     center of gravity.
   * @param JKgMetersSquared The moment of inertia of the robot.
   * @param G The gearing reduction as output over input.
   * @return A LinearSystem representing a differential drivetrain.
   * @throws IllegalArgumentException if m &lt;= 0, r &lt;= 0, ocfg &lt; 0, J &lt;= 0, or G &lt;= 0.
   */
  public static LinearSystem<N1, N1, N1> createHDrivetrainLateralVelocitySystem(
    ...
    if (offsetFromCenterOfGravityMeters < 0.0) {
      throw new IllegalArgumentException(
          "offsetFromCenterOfGravityMeters must not be less than zero.");
    }
    if (JKgMetersSquared <= 0.0) {
      throw new IllegalArgumentException("JKgMetersSquared must be greater than zero.");
    }
    ...
    final double C3 =
        1 / massKg
            + offsetFromCenterOfGravityMeters * offsetFromCenterOfGravityMeters / JKgMetersSquared;
  ...
  /**
   * Identify a H-drive lateral plant given the lateral plants's kV and kA in both linear, This is
   * used in combination with a regular Differential Drive Velocity System to create an H-Drive
   * {(volts/(meter/sec), (volts/(meter/sec²))} and angular {(volts/(radian/sec)),
   * (volts/(radian/sec²))} cases. These constants can be found using SysId.
   *
   * <p>States: [[lateral velocity]]<br>
   ...
   * @see <a href= "https://github.com/wpilibsuite/sysid">https://github.com/wpilibsuite/sysid</a>
   */
  public static LinearSystem<N1, N1, N1> identifyHDriveLateralVelocitySystem(
      double kVLinear, double kALinear, double kVAngular, double kAAngular) {
    if (kVLinear <= 0.0) {
      throw new IllegalArgumentException("Kv, linear must be greater than zero.");
    }
