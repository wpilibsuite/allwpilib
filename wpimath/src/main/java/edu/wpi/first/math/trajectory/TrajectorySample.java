...
public class TrajectorySample implements Interpolatable<TrajectorySample>, StructSerializable {
     ...
     */
    public TrajectorySample(Time timestamp, Pose2d pose, ChassisSpeeds vel, ChassisAccelerations accel) {
    ...
    public final boolean equals(Object o) {
        ...
        return timestamp.equals(that.timestamp) && pose.equals(that.pose) && vel.equals(that.vel) && accel.equals(that.accel);
    }
    ...
    public TrajectorySample integrate(Time dt) {
        ...
        var newVel = new  ChassisSpeeds(
                vel.vx + accel.ax * dts,
                vel.vy + accel.ay * dts,
                vel.omega + accel.alpha * dts
        );
    ...
    public TrajectorySample interpolate(TrajectorySample end, double t) {
        ...
        // aₖ₊₁ = aₖ + jΔt
        var newAccel = new ChassisAccelerations(
        ...
        var newPose = new Pose2d(
                pose.getX() + vel.vx * interpDt + 0.5 * accel.ax * interpDt * interpDt + (1.0/6) * jerk.ax * Math.pow(interpDt, 3),
                pose.getY() + vel.vy * interpDt + 0.5 * accel.ay * interpDt * interpDt + (1.0/6) * jerk.ay * Math.pow(interpDt, 3),
                new Rotation2d(pose.getRotation().getRadians() + vel.omega * interpDt + 0.5 * accel.alpha * interpDt * interpDt + (1.0/6) * jerk.alpha * Math.pow(interpDt, 3))
        );
