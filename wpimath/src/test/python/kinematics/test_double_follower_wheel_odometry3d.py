...
def _make_odometry():
    return DoubleFollowerWheelOdometry3d(
        X_WHEEL_Y_POS, Y_WHEEL_X_POS, _zero_rotation(), DoubleFollowerWheelPositions()
    )


def test_multiple_consecutive_updates():
    odometry = _make_odometry()
    wheel_positions = DoubleFollowerWheelPositions(1, 1)
...
def test_two_iterations():
    odometry = _make_odometry()
    wheel_positions = DoubleFollowerWheelPositions(0.1, 0)

    odometry.resetPosition(_zero_rotation(), DoubleFollowerWheelPositions(), _zero_pose())

    odometry.update(_zero_rotation(), DoubleFollowerWheelPositions())
...
def test_gyro_angle_reset():
    ...
    odometry.resetPosition(gyro, DoubleFollowerWheelPositions(), Pose3d(0, 0, 0, field_angle))

    positions = DoubleFollowerWheelPositions(1, 0)
...
def test_gyro_offset():
    odometry = _make_odometry()
    wheel_positions = DoubleFollowerWheelPositions()
