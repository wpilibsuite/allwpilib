...
def _make_odometry():
    return ThreeDeadWheelOdometry3d(
        X_WHEEL_1_Y_POS,
        X_WHEEL_2_Y_POS,
        Y_WHEEL_X_POS,
        _zero_rotation(),
...
def test_multiple_consecutive_updates():
    ...
    wheel_positions = ThreeDeadWheelPositions(1, 1, 1)
...
def test_two_iterations():
    odometry = _make_odometry()
    wheel_positions = ThreeDeadWheelPositions(0.1, 0.1, 0)

    odometry.resetPosition(_zero_rotation(), ThreeDeadWheelPositions(), _zero_pose())

    odometry.update(_zero_rotation(), ThreeDeadWheelPositions())
...
def test_gyro_angle_reset():
    ...
    odometry.resetPosition(
        gyro, ThreeDeadWheelPositions(), Pose3d(0, 0, 0, field_angle)
    )

    positions = ThreeDeadWheelPositions(1, 1, 0)
...
def test_gyro_offset():
    odometry = _make_odometry()
    wheel_positions = ThreeDeadWheelPositions()
