...
static_assert(
    std::same_as<decltype(wpi::tunables::TunableRegistry::GetTuneRevision(
                     std::declval<const wpi::tunables::Tunable<
                         wpi::units::meter_t>&>())),
                 uint64_t>);
...
TEST_CASE_METHOD(UnitsTunableTest, "UnitsTunableTest RevisionForUnitTunable",
  ...
  auto revision = wpi::tunables::TunableRegistry::GetTuneRevision(distance);
  CHECK(revision == 0);
  CHECK(wpi::tunables::TunableRegistry::GetTuneRevision(constDistance) == 0);

  distance = wpi::units::meter_t{2.0};
  CHECK(distance.Get().to<double>() == 2.0);
  CHECK(wpi::tunables::TunableRegistry::GetTuneRevision(distance) == 0);
  ...
  CHECK(wpi::tunables::TunableRegistry::GetTuneRevision(distance) == 1);
  CHECK(wpi::tunables::TunableRegistry::GetTuneRevision(constDistance) == 1);
}
...
TEST_CASE_METHOD(UnitsTunableTest,
  ...
  REQUIRE(uid);
  CHECK(TunableRegistry::GetTuneRevision(complex) == 0);

  backend->SetDouble("/unit/distance", 2.0);
  TunableRegistry::Update();

  CHECK(complex.distance.to<double>() == 2.0);
  CHECK(TunableRegistry::GetTuneRevision(complex) == 1);
