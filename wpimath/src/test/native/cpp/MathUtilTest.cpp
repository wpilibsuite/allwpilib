...
TEST(MathUtilTest, ApplyDeadbandUnits) {
  // < 0
  EXPECT_DOUBLE_EQ(
      -20, wpi::math::ApplyDeadband<wpi::units::radian_t>(-20_rad, 1_rad, 20_rad).value());
}
...
TEST(MathUtilTest, CopySignPowWithUnits) {
  EXPECT_DOUBLE_EQ(
      0, wpi::math::CopySignPow<wpi::units::meters_per_second_t>(0_mps, 2.0).value());
  EXPECT_DOUBLE_EQ(
      1, wpi::math::CopySignPow<wpi::units::meters_per_second_t>(1_mps, 2.0).value());
  EXPECT_DOUBLE_EQ(
      -1, wpi::math::CopySignPow<wpi::units::meters_per_second_t>(-1_mps, 2.0).value());

  EXPECT_DOUBLE_EQ(
      0.5 * 0.5 * 10,
      wpi::math::CopySignPow<wpi::units::meters_per_second_t>(5_mps, 2.0, 10_mps).value());
  EXPECT_DOUBLE_EQ(
      -0.5 * 0.5 * 10,
      wpi::math::CopySignPow<wpi::units::meters_per_second_t>(-5_mps, 2.0, 10_mps)
          .value());
}
