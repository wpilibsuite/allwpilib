...
TEST(MathUtilTest, ApplyDeadband2dUnits) {
  ...
  EXPECT_EQ(
      Eigen::Vector2d(0_mps, 0_mps),
      frc::ApplyDeadband(Eigen::Vector2d(0_mps, 0_mps), 0.02_mps, 2.5_mps));
}
...
TEST(MathUtilTest, CopySignPow2dUnits) {
  ...
  EXPECT_EQ(Eigen::Vector2d(0_mps, 0_mps),
            frc::CopySignPow(Eigen::Vector2d(0_mps, 0_mps), 2.0, 5_mps));
