...
#define CHECK_UNITS_EQ(a, b) CHECK((a).value() == Catch::Approx((b).value()))

#define CHECK_UNITS_NEAR(a, b, c) \
  CHECK((a).value() == Catch::Approx((b).value()).margin(c))

TEST_CASE("MathUtilTest ApplyDeadbandUnityScale", "[wpimath]") {
  // < 0
  CHECK(-1.0 == Catch::Approx(wpi::math::ApplyDeadband(-1.0, 0.02)));
  CHECK((-0.03 + 0.02) / (1.0 - 0.02) ==
        Catch::Approx(wpi::math::ApplyDeadband(-0.03, 0.02)));
  CHECK(0.0 == Catch::Approx(wpi::math::ApplyDeadband(-0.02, 0.02)));
  CHECK(0.0 == Catch::Approx(wpi::math::ApplyDeadband(-0.01, 0.02)));

  // == 0
  CHECK(0.0 == Catch::Approx(wpi::math::ApplyDeadband(0.0, 0.02)));
...
TEST_CASE("MathUtilTest ApplyDeadbandArbitraryScale", "[wpimath]") {
  // < 0
  CHECK(-2.5 == Catch::Approx(wpi::math::ApplyDeadband(-2.5, 0.02, 2.5)));
  CHECK(0.0 == Catch::Approx(wpi::math::ApplyDeadband(-0.02, 0.02, 2.5)));
  CHECK(0.0 == Catch::Approx(wpi::math::ApplyDeadband(-0.01, 0.02, 2.5)));

  // == 0
  CHECK(0.0 == Catch::Approx(wpi::math::ApplyDeadband(0.0, 0.02, 2.5)));
...
TEST_CASE("MathUtilTest ApplyDeadbandLargeMaxMagnitude", "[wpimath]") {
  CHECK(80.0 == Catch::Approx(wpi::math::ApplyDeadband(
                    100.0, 20.0, std::numeric_limits<double>::infinity())));
}
...
TEST_CASE("MathUtilTest CopyDirectionPow", "[wpimath]") {
  CHECK(0.5 == Catch::Approx(wpi::math::CopyDirectionPow(0.5, 1.0)));
  CHECK(-0.5 == Catch::Approx(wpi::math::CopyDirectionPow(-0.5, 1.0)));

  CHECK(0.5 * 0.5 == Catch::Approx(wpi::math::CopyDirectionPow(0.5, 2.0)));
  CHECK(-(0.5 * 0.5) == Catch::Approx(wpi::math::CopyDirectionPow(-0.5, 2.0)));

  CHECK(std::sqrt(0.5) == Catch::Approx(wpi::math::CopyDirectionPow(0.5, 0.5)));
  CHECK(-std::sqrt(0.5) ==
        Catch::Approx(wpi::math::CopyDirectionPow(-0.5, 0.5)));

  CHECK(0.0 == Catch::Approx(wpi::math::CopyDirectionPow(0.0, 2.0)));
  CHECK(1.0 == Catch::Approx(wpi::math::CopyDirectionPow(1.0, 2.0)));
  CHECK(-1.0 == Catch::Approx(wpi::math::CopyDirectionPow(-1.0, 2.0)));

  CHECK(std::pow(0.8, 0.3) ==
        Catch::Approx(wpi::math::CopyDirectionPow(0.8, 0.3)));
  CHECK(-std::pow(0.8, 0.3) ==
        Catch::Approx(wpi::math::CopyDirectionPow(-0.8, 0.3)));
}

TEST_CASE("MathUtilTest CopyDirectionPowWithMaxMagnitude", "[wpimath]") {
  CHECK(5.0 == Catch::Approx(wpi::math::CopyDirectionPow(5.0, 1.0, 10.0)));
  CHECK(-5.0 == Catch::Approx(wpi::math::CopyDirectionPow(-5.0, 1.0, 10.0)));

  CHECK(0.5 * 0.5 * 10 ==
        Catch::Approx(wpi::math::CopyDirectionPow(5.0, 2.0, 10.0)));
  CHECK(-0.5 * 0.5 * 10 ==
        Catch::Approx(wpi::math::CopyDirectionPow(-5.0, 2.0, 10.0)));

  CHECK(std::sqrt(0.5) * 10 ==
        Catch::Approx(wpi::math::CopyDirectionPow(5.0, 0.5, 10.0)));
  CHECK(-std::sqrt(0.5) * 10 ==
        Catch::Approx(wpi::math::CopyDirectionPow(-5.0, 0.5, 10.0)));

  CHECK(0.0 == Catch::Approx(wpi::math::CopyDirectionPow(0.0, 2.0, 5.0)));
  CHECK(5.0 == Catch::Approx(wpi::math::CopyDirectionPow(5.0, 2.0, 5.0)));
  CHECK(-5.0 == Catch::Approx(wpi::math::CopyDirectionPow(-5.0, 2.0, 5.0)));

  CHECK(std::pow(0.8, 0.3) * 100 ==
        Catch::Approx(wpi::math::CopyDirectionPow(80.0, 0.3, 100.0)));
  CHECK(-std::pow(0.8, 0.3) * 100 ==
        Catch::Approx(wpi::math::CopyDirectionPow(-80.0, 0.3, 100.0)));
}
...
TEST_CASE("MathUtilTest InputModulus", "[wpimath]") {
  ...
  CHECK(-20.0 == Catch::Approx(
                     wpi::math::InputModulus(170.0 - (-170.0), -180.0, 180.0)));
  CHECK(-20.0 == Catch::Approx(wpi::math::InputModulus(170.0 + 360.0 - (-170.0),
                                                       -180.0, 180.0)));
  CHECK(-20.0 == Catch::Approx(wpi::math::InputModulus(170.0 - (-170.0 + 360.0),
                                                       -180.0, 180.0)));
  CHECK(20.0 ==
        Catch::Approx(wpi::math::InputModulus(-170.0 - 170.0, -180.0, 180.0)));
  CHECK(20.0 == Catch::Approx(wpi::math::InputModulus(-170.0 + 360.0 - 170.0,
                                                      -180.0, 180.0)));
  CHECK(20.0 == Catch::Approx(wpi::math::InputModulus(-170.0 - (170.0 + 360.0),
                                                      -180.0, 180.0)));
  ...
  CHECK(340.0 ==
        Catch::Approx(wpi::math::InputModulus(170.0 - 190.0, 0.0, 360.0)));
  CHECK(340.0 == Catch::Approx(wpi::math::InputModulus(170.0 + 360.0 - 190.0,
                                                       0.0, 360.0)));
  CHECK(340.0 == Catch::Approx(wpi::math::InputModulus(170.0 - (190.0 + 360.0),
                                                       0.0, 360.0)));
  ...
  CHECK(2.0 == Catch::Approx(wpi::math::InputModulus(0.0, 1.0, 3.0)));
  CHECK(3.0 == Catch::Approx(wpi::math::InputModulus(1.0, 1.0, 3.0)));
  CHECK(2.0 == Catch::Approx(wpi::math::InputModulus(2.0, 1.0, 3.0)));
  CHECK(3.0 == Catch::Approx(wpi::math::InputModulus(3.0, 1.0, 3.0)));
  CHECK(2.0 == Catch::Approx(wpi::math::InputModulus(4.0, 1.0, 3.0)));

  // Test all supported types
  CHECK(-20.0 == Catch::Approx(wpi::math::InputModulus<double>(170.0 - (-170.0),
                                                               -170.0, 190.0)));
