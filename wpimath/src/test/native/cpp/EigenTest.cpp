...
TEST(EigenTest, Multiplication) {
  wpimath::Matrixd<2, 2> m1{{2, 1}, {0, 1}};
  wpimath::Matrixd<2, 2> m2{{3, 0}, {0, 2.5}};

  const auto result = m1 * m2;

  wpimath::Matrixd<2, 2> expectedResult{{6.0, 2.5}, {0.0, 2.5}};

  EXPECT_TRUE(expectedResult.isApprox(result));

  wpimath::Matrixd<2, 3> m3{{1.0, 3.0, 0.5}, {2.0, 4.3, 1.2}};
  wpimath::Matrixd<3, 4> m4{
      {3.0, 1.5, 2.0, 4.5}, {2.3, 1.0, 1.6, 3.1}, {5.2, 2.1, 2.0, 1.0}};

  const auto result2 = m3 * m4;

  wpimath::Matrixd<2, 4> expectedResult2{{12.5, 5.55, 7.8, 14.3},
