...
#include "wpimath/kinematics/MecanumDriveOdometry3d.h"
...
class MecanumDriveOdometry3dTest : public ::testing::Test {
 protected:
  ...
  MecanumDriveOdometry3d odometry{kinematics, wpimath::Rotation3d{}, zero};
};

TEST_F(MecanumDriveOdometry3dTest, Initialize) {
  MecanumDriveOdometry3d odometry{
      kinematics, wpimath::Rotation3d{}, zero,
...
TEST_F(MecanumDriveOdometry3dTest, MultipleConsecutiveUpdates) {
  ...
  odometry.ResetPosition(wpimath::Rotation3d{}, wheelDeltas, Pose3d{});

  odometry.Update(wpimath::Rotation3d{}, wheelDeltas);
  auto secondPose = odometry.Update(wpimath::Rotation3d{}, wheelDeltas);
...
TEST_F(MecanumDriveOdometry3dTest, TwoIterations) {
  odometry.ResetPosition(wpimath::Rotation3d{}, zero, Pose3d{});
  MecanumDriveWheelPositions wheelDeltas{0.3536_m, 0.3536_m, 0.3536_m,
                                         0.3536_m};

  odometry.Update(wpimath::Rotation3d{}, MecanumDriveWheelPositions{});
  auto pose = odometry.Update(wpimath::Rotation3d{}, wheelDeltas);
...
TEST_F(MecanumDriveOdometry3dTest, 90DegreeTurn) {
  ...
  odometry.Update(wpimath::Rotation3d{}, MecanumDriveWheelPositions{});
  auto pose =
      odometry.Update(wpimath::Rotation3d{0_deg, 0_deg, 90_deg}, wheelDeltas);
...
TEST_F(MecanumDriveOdometry3dTest, GyroAngleReset) {
  odometry.ResetPosition(wpimath::Rotation3d{0_deg, 0_deg, 90_deg}, zero,
                         Pose3d{});
...
TEST_F(MecanumDriveOdometry3dTest, AccuracyFacingTrajectory) {
  ...
  wpimath::MecanumDriveOdometry3d odometry{kinematics, wpimath::Rotation3d{},
                                           wheelPositions};
...
TEST_F(MecanumDriveOdometry3dTest, AccuracyFacingXAxis) {
  ...
  while (t < trajectory.TotalTime()) {
    ...
    auto xhat = odometry.Update(
        wpimath::Rotation3d{0_rad, 0_rad, distribution(generator) * 0.05_rad},
        wheelPositions);
    double error = groundTruthState.pose.Translation()
                       .Distance(xhat.Translation().ToTranslation2d())
                       .value();
