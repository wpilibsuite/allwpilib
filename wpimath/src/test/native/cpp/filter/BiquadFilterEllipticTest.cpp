...
TEST(BiquadFilterEllipticTest, LowPass4thOrderMatchesScipy) {
  ...
  auto filter =
      BiquadFilter::Elliptic(Kind::LowPass, 4, 1000.0, 50.0, 0.0, 1.0, 40.0);
...
TEST(BiquadFilterEllipticTest, LowPassOddOrder5HasFirstOrderSection) {
  ...
  auto filter =
      BiquadFilter::Elliptic(Kind::LowPass, 5, 1000.0, 50.0, 0.0, 1.0, 40.0);
  auto sections = filter.Sections();
...
TEST(BiquadFilterEllipticTest, LowPassEquirippleInPassbandAndStopband) {
  ...
  auto filter = BiquadFilter::Elliptic(Kind::LowPass, 4, 1000.0, 50.0, 0.0,
                                       kRipple, kAtten);
  auto sections = filter.Sections();
...
TEST(BiquadFilterEllipticTest, HighPassResponse) {
  ...
  constexpr double kRipple = 1.0;
  constexpr double kAtten = 40.0;
  auto filter = BiquadFilter::Elliptic(Kind::HighPass, 4, 1000.0, 100.0, 0.0,
                                       kRipple, kAtten);
  auto sections = filter.Sections();
...
TEST(BiquadFilterEllipticTest, BandPass4thOrderMatchesScipy) {
  ...
  // fs=1000.0)
  auto filter =
      BiquadFilter::Elliptic(Kind::BandPass, 4, 1000.0, 80.0, 120.0, 1.0, 40.0);
  auto sections = filter.Sections();
...
TEST(BiquadFilterEllipticTest, BandPassResponse) {
  constexpr double kRipple = 1.0;
  constexpr double kAtten = 40.0;
  auto filter = BiquadFilter::Elliptic(Kind::BandPass, 3, 1000.0, 80.0, 120.0,
                                       kRipple, kAtten);
  auto sections = filter.Sections();
...
TEST(BiquadFilterEllipticTest, RejectsInvalidArgs) {
  ...
  EXPECT_THROW(
      BiquadFilter::Elliptic(Kind::LowPass, 0, 1000.0, 50.0, 0.0, 1.0, 40.0),
      std::invalid_argument);
  EXPECT_THROW(
      BiquadFilter::Elliptic(Kind::LowPass, 4, 0.0, 50.0, 0.0, 1.0, 40.0),
      std::invalid_argument);
  EXPECT_THROW(
      BiquadFilter::Elliptic(Kind::LowPass, 4, 1000.0, 50.0, 0.0, 0.0, 40.0),
      std::invalid_argument);
  // Stopband must be deeper than passband ripple.
  EXPECT_THROW(
      BiquadFilter::Elliptic(Kind::LowPass, 4, 1000.0, 50.0, 0.0, 40.0, 1.0),
      std::invalid_argument);
  EXPECT_THROW(
      BiquadFilter::Elliptic(Kind::LowPass, 4, 1000.0, 50.0, 0.0, 5.0, 5.0),
      std::invalid_argument);
  // Frequencies out of range / inverted.
  EXPECT_THROW(
      BiquadFilter::Elliptic(Kind::LowPass, 4, 1000.0, 600.0, 0.0, 1.0, 40.0),
      std::invalid_argument);
  EXPECT_THROW(
      BiquadFilter::Elliptic(Kind::BandPass, 4, 1000.0, 120.0, 80.0, 1.0, 40.0),
      std::invalid_argument);
}
