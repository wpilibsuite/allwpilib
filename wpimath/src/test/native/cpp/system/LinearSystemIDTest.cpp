...
TEST(LinearSystemIDTest, IdentifyPositionSystem) {
  // By controls engineering in wpi::math,
  // x-dot = [0 1 | 0 -kv/ka] x = [0 | 1/ka] u
  constexpr double kv = 1.0;
  constexpr double ka = 0.5;

#if __GNUC__ <= 11
  auto model = wpi::math::LinearSystemId::IdentifyPositionSystem<units::meter>(
      kv * 1_V / 1_mps, ka * 1_V / 1_mps_sq);
#else
  constexpr auto model =
      wpi::math::LinearSystemId::IdentifyPositionSystem<units::meter>(
          kv * 1_V / 1_mps, ka * 1_V / 1_mps_sq);
#endif
...
#if __GNUC__ <= 11
  auto model = wpi::math::LinearSystemId::IdentifyVelocitySystem<units::meter>(
      kv * 1_V / 1_mps, ka * 1_V / 1_mps_sq);
#else
  constexpr auto model =
      wpi::math::LinearSystemId::IdentifyVelocitySystem<units::meter>(
          kv * 1_V / 1_mps, ka * 1_V / 1_mps_sq);
#endif
