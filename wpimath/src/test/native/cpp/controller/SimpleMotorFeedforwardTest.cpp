...
TEST(SimpleMotorFeedforwardTest, CalculateVoltage) {
...
TEST(SimpleMotorFeedforwardTest, CalculateCurrent) {
  constexpr auto Ks = 0.5_A;
  constexpr auto Kv = 3_A / 1_mps;
  ...
  EXPECT_NEAR((37.524995834325161_A + Ks).value(),
              simpleMotor.Calculate(2_mps, 3_mps).value(), 0.002);
  EXPECT_NEAR(plantInversion.Calculate(r, nextR)(0) + Ks.value(),
              simpleMotor.Calculate(2_mps, 3_mps).value(), 0.002);
...
TEST(SimpleMotorFeedforwardTest, NegativeVoltageGains) {
  constexpr auto Ks = 0.5_V;
  constexpr auto Kv = -3_V / 1_mps;
  constexpr auto Ka = -0.6_V / 1_mps_sq;
...
TEST(SimpleMotorFeedforwardTest, NegativeCurrentGains) {
  constexpr auto Ks = 0.5_A;
  constexpr auto Kv = -3_A / 1_mps;
  constexpr auto Ka = -0.6_A / 1_mps_sq;
  constexpr units::second_t dt = 0_ms;
