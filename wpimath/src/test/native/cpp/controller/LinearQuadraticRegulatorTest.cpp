...
TEST_CASE("LinearQuadraticRegulatorTest ElevatorGains", "[wpimath]") {
  ...
  CHECK(522.87006795347486 == Catch::Approx(K(0, 0)).margin(1e-6));
  CHECK(38.239878385020411 == Catch::Approx(K(0, 1)).margin(1e-6));
}
...
TEST_CASE("LinearQuadraticRegulatorTest ArmGains", "[wpimath]") {
  ...
  CHECK(19.339349883583761 == Catch::Approx(K(0, 0)).margin(1e-6));
  CHECK(3.3542559517421582 == Catch::Approx(K(0, 1)).margin(1e-6));
}
...
TEST_CASE("LinearQuadraticRegulatorTest FourMotorElevator", "[wpimath]") {
  ...
  CHECK(10.38 == Catch::Approx(K(0, 0)).margin(1e-1));
  CHECK(0.69 == Catch::Approx(K(0, 1)).margin(1e-1));
}
...
TEST_CASE("LinearQuadraticRegulatorTest MatrixOverloadsWithSingleIntegrator",
  ...
  CHECK(0.99750312499512261 == Catch::Approx(K(0, 0)).margin(1e-10));
  CHECK(0.0 == Catch::Approx(K(0, 1)).margin(1e-10));
  CHECK(0.0 == Catch::Approx(K(1, 0)).margin(1e-10));
  CHECK(0.99750312499512261 == Catch::Approx(K(1, 1)).margin(1e-10));

  // QRN overload
  Matrixd<2, 2> N{Matrixd<2, 2>::Identity()};
  Matrixd<2, 2> Kimf = LinearQuadraticRegulator<2, 2>{A, B, Q, R, N, 5_ms}.K();
  CHECK(1.0 == Catch::Approx(Kimf(0, 0)).margin(1e-10));
  CHECK(0.0 == Catch::Approx(Kimf(0, 1)).margin(1e-10));
  CHECK(0.0 == Catch::Approx(Kimf(1, 0)).margin(1e-10));
  CHECK(1.0 == Catch::Approx(Kimf(1, 1)).margin(1e-10));
}
...
TEST_CASE("LinearQuadraticRegulatorTest MatrixOverloadsWithDoubleIntegrator",
  ...
  CHECK(1.9960017786537287 == Catch::Approx(K(0, 0)).margin(1e-10));
  CHECK(0.51182128351092726 == Catch::Approx(K(0, 1)).margin(1e-10));

  // QRN overload
  Matrixd<2, 2> Aref{{0, 1}, {0, -Kv / (Ka * 5.0)}};
  Matrixd<1, 2> Kimf = GetImplicitModelFollowingK<2, 1>(A, B, Q, R, Aref, 5_ms);
  CHECK(0.0 == Catch::Approx(Kimf(0, 0)).margin(1e-10));
  CHECK(-6.9190500116751458e-05 == Catch::Approx(Kimf(0, 1)).margin(1e-10));
}
...
TEST_CASE("LinearQuadraticRegulatorTest LatencyCompensate", "[wpimath]") {
  ...
  CHECK(8.97115941 == Catch::Approx(controller.K(0, 0)).margin(1e-3));
  CHECK(0.07904881 == Catch::Approx(controller.K(0, 1)).margin(1e-3));
}
