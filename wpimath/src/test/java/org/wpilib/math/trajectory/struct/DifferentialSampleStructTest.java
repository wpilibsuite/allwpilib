...
class DifferentialSampleStructTest {
  ...
  @Test
  void testRoundtrip() {
    ByteBuffer buffer = ByteBuffer.allocate(DifferentialSample.struct.getSize());
    buffer.order(ByteOrder.LITTLE_ENDIAN);
    DifferentialSample.struct.pack(buffer, DATA);
    buffer.rewind();

    assertEquals(DATA, DifferentialSample.struct.unpack(buffer));
  }
}
