...
class GradientTest {
  ...
  void testIdenticalVariables() {
    ...
    assertEquals(
        2 * a.value() * new Gradient(x, a).value().get(0, 0) + 1,
        new Gradient(c, a).value().get(0, 0));
  }
  ...
  void testAtan2() {
    ...
    assertEquals(g.value().get(0, 0), -2.0 / (2 * 2 + x.value() * x.value()), 1e-15);
  ...
  void testSign() {
    ...
    // sgn(1.0)
    x.setValue(1.0);
    assertEquals(sign(x.value()), Variable.sign(x).value());

    var g = new Gradient(Variable.sign(x), x);
    assertEquals(0.0, g.get().value().get(0, 0));
    assertEquals(0.0, g.value().get(0, 0));
    ...
    g = new Gradient(Variable.sign(x), x);
    assertEquals(0.0, g.get().value().get(0, 0));
    assertEquals(0.0, g.value().get(0, 0));
