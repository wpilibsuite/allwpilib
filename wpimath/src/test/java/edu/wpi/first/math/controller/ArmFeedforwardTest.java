...
class ArmFeedforwardTest {
  ...
  void testCalculate() {
    ...
    final double u = m_armFF.calculate(currentAngleRadians, currentVelocityRadPerSec, nextVelocityRadPerSec, dtSeconds);
    final Matrix<N2, N2> A = new Matrix<>(Nat.N2(), Nat.N2(), new double[] { 0.0, 1.0, 0.0, -kv / ka });
    final Matrix<N2, N1> B = new Matrix<>(Nat.N2(), Nat.N1(), new double[] { 0.0, 1.0 / ka });
    final BiFunction<Matrix<N2, N1>, Matrix<N1, N1>, Matrix<N2, N1>> f = (x_, u_) -> {
      Matrix<N2, N1> c = MatBuilder.fill(Nat.N2(), Nat.N1(), 0.0,
          Math.signum(x_.get(1, 0)) * (-ks / ka) - (kg / ka) * Math.cos(x_.get(0, 0)));
      return A.times(x_).plus(B.times(u_)).plus(c);
    };
    final Matrix<N2, N1> actual_x_k1 = NumericalIntegration.rk4(
        f,
