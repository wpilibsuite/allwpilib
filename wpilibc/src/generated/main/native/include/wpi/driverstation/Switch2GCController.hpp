 ...
 *
 * This class handles Switch2GC input that comes from the Driver Station.
 * Each time a value is requested the most recent value is returned.
 */
class Switch2GCController
    : public wpi::util::Sendable,
      public wpi::util::SendableHelper<Switch2GCController> {
 public:
   ...
   *             into.
   */
  explicit Switch2GCController(int port);

  ~Switch2GCController() override = default;

  Switch2GCController(Switch2GCController&&) = default;
  Switch2GCController& operator=(Switch2GCController&&) = default;
   ...
   * @return Whether the button was released since the last check.
   */
  bool GetZLButtonReleased();

  /**
   * Constructs an event instance around the ZL button's
   * digital signal.
   *
   * @param loop the event loop instance to attach the event to.
   * @return an event instance representing the ZL button's
   * digital signal attached to the given loop.
   */
  BooleanEvent ZL(EventLoop* loop) const;
