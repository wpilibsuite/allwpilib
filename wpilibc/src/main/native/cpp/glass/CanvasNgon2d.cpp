...
constexpr size_t kNumSidesOff = kRadiusOff + 4;
constexpr size_t kWeightOff = kNumSidesOff + 4;
constexpr size_t kFillOff = kWeightOff + 4;
constexpr size_t kColorOff = kFillOff + 1;
constexpr size_t kZOrderOff = kColorOff + 4;
}  // namespace

using StructType = wpi::Struct<frc::CanvasNgon2d>;

frc::CanvasNgon2d StructType::Unpack(std::span<const uint8_t> data) {
  return frc::CanvasNgon2d{
      wpi::UnpackStruct<float, kXOff>(data),
      wpi::UnpackStruct<float, kYOff>(data),
      wpi::UnpackStruct<float, kRadiusOff>(data),
      wpi::UnpackStruct<int, kNumSidesOff>(data),
...
void StructType::Pack(std::span<uint8_t> data, const frc::CanvasNgon2d& value) {
  wpi::PackStruct<kXOff>(data, value.x);
  wpi::PackStruct<kYOff>(data, value.y);
  wpi::PackStruct<kRadiusOff>(data, value.radius);
  wpi::PackStruct<kNumSidesOff>(data, value.numSides);
