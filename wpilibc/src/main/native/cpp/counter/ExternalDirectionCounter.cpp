...
ExternalDirectionCounter::ExternalDirectionCounter(
    ...
    : m_countSource{countSource, NullDeleter<DigitalSource>()},
      m_directionSource{directionSource, NullDeleter<DigitalSource>()} {
...
void ExternalDirectionCounter::InitExternalDirectionCounter() {
  ...
  m_handle = HAL_InitializeCounter(
      HAL_Counter_Mode::HAL_Counter_kExternalDirection, &index, &status);
  ...
  HAL_SetCounterUpSource(
      m_handle, m_countSource->GetPortHandleForRouting(),
      (HAL_AnalogTriggerType)m_countSource->GetAnalogTriggerTypeForRouting(),
      &status);
  wpi_setHALError(status);
  ...
  SendableRegistry::GetInstance().AddLW(this, "External Direction Counter",
                                        index);
}
