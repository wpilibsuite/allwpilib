...
struct MatchDataSender {
  MatchDataSender()
    ...
    int32_t wordInt = 0;
    std::memcpy(&ctlWord, &wordInt, sizeof(ctlWord));
    controlWord.Set(ctlWord);
    opMode.Set("");
  }
  ...
  int32_t prevControlWordInt = 0;
  int64_t prevOpModeId = 0;
};
...
class DataLogSender {
 ...
 private:
  ...
  int32_t m_prevControlWord;
  int64_t m_prevOpModeId;
...
RobotMode DriverStation::GetRobotMode() {
  HAL_ControlWord controlWord;
  HAL_GetControlWord(&controlWord);
  return static_cast<RobotMode>(controlWord.robotMode);
}

bool DriverStation::IsAutonomous() {
  HAL_ControlWord controlWord;
  HAL_GetControlWord(&controlWord);
  return controlWord.robotMode == HAL_ROBOTMODE_AUTONOMOUS;
}
...
bool DriverStation::IsTeleop() {
  HAL_ControlWord controlWord;
  HAL_GetControlWord(&controlWord);
  return controlWord.robotMode == HAL_ROBOTMODE_TELEOPERATED;
}
...
bool DriverStation::IsTest() {
  HAL_ControlWord controlWord;
  HAL_GetControlWord(&controlWord);
  return controlWord.robotMode == HAL_ROBOTMODE_TEST;
}

bool DriverStation::IsTestEnabled() {
  HAL_ControlWord controlWord;
  HAL_GetControlWord(&controlWord);
  return controlWord.robotMode == HAL_ROBOTMODE_TEST && controlWord.enabled;
}
...
static int64_t DoAddOpMode(RobotMode mode, std::string_view name,
  ...
  for (;;) {
    int64_t id = HAL_OPMODE_MAKE_ID(static_cast<int>(mode),
    ...
    if (HAL_OPMODE_GET_MODE(it->second.id) == static_cast<int>(mode) &&
...
void SendMatchData() {
  ...
  int32_t wordInt = 0;
  std::memcpy(&wordInt, &ctlWord, sizeof(ctlWord));
  if (wordInt != inst.matchDataSender.prevControlWordInt) {
    inst.matchDataSender.prevControlWordInt = wordInt;
    inst.matchDataSender.controlWord.Set(ctlWord);
  }

  int64_t opModeId = HAL_GetOpMode();
...
void DataLogSender::Init(wpi::log::DataLog& log, bool logJoysticks,
  ...
  HAL_ControlWord ctlWord;
  HAL_GetControlWord(&ctlWord);
  std::memcpy(&m_prevControlWord, &ctlWord, sizeof(ctlWord));
...
void DataLogSender::Send(uint64_t timestamp) {
  ...
  int32_t wordInt = 0;
  std::memcpy(&wordInt, &ctlWord, sizeof(ctlWord));
  if (wordInt != m_prevControlWord) {
    m_prevControlWord = wordInt;
    m_logControlWord.Append(ctlWord);
  }
