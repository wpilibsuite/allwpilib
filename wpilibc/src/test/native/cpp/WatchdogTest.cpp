...
TEST(WatchdogTest, EnableDisable) {
  ...
  Watchdog watchdog(0.2, [&] { watchdogCounter++; });
...
TEST(WatchdogTest, SetTimeout) {
  ...
  Watchdog watchdog(0.5, [&] { watchdogCounter++; });

  watchdog.Enable();
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
  watchdog.SetTimeout(0.1);

  EXPECT_EQ(0.1, watchdog.GetTimeout());
  EXPECT_EQ(0u, watchdogCounter) << "Watchdog triggered early";
...
TEST(WatchdogTest, MultiWatchdog) {
  ...
  Watchdog watchdog1(0.1, [&] { watchdogCounter1++; });
  Watchdog watchdog2(0.3, [&] { watchdogCounter2++; });
