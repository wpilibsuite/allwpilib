...
TEST(ElevatorSimTest, StateSpaceSim) {
  sim::ElevatorSim sim(DCMotor::Vex775Pro(4), 14.67, 8_kg, 0.75_in, 0_m, 3_m,
                       true, 0_m, {0.01});
...
TEST(ElevatorSimTest, InitialState) {
  ...
  sim::ElevatorSim sim(DCMotor::KrakenX60(2), 20, 8_kg, 0.1_m, 0_m, 1_m, true,
...
TEST(ElevatorSimTest, MinMax) {
  sim::ElevatorSim sim(DCMotor::Vex775Pro(4), 14.67, 8_kg, 0.75_in, 0_m, 1_m,
                       true, 0_m, {0.01});
  for (size_t i = 0; i < 100; ++i) {
...
TEST(ElevatorSimTest, Stability) {
  ...
  LinearSystem<2, 1, 1> system =
      LinearSystemId::ElevatorSystem(DCMotor::Vex775Pro(4), 4_kg, 0.5_in, 100)
          .Slice(0);
  EXPECT_NEAR_UNITS(units::meter_t{system.CalculateX(
                        Vectord<2>{0.0, 0.0}, Vectord<1>{12.0}, 20_ms * 50)(0)},
