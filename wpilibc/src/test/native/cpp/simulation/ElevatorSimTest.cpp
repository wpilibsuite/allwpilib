...
TEST(ElevatorSimTest, StateSpaceSim) {
  frc::sim::ElevatorSim sim = frc::sim::ElevatorSim::Create(
      frc::DCMotor::Vex775Pro(4), 14.67, 8_kg, 0.75_in, 0_m, 3_m, true, 0_m,
      9.8_mps_sq, {0.01});
...
TEST(ElevatorSimTest, MinMax) {
  frc::sim::ElevatorSim sim = frc::sim::ElevatorSim::Create(
      frc::DCMotor::Vex775Pro(4), 14.67, 8_kg, 0.75_in, 0_m, 1_m, true, 0_m,
      9.8_mps_sq, {0.01});
  for (size_t i = 0; i < 100; ++i) {
...
TEST(ElevatorSimTest, Stability) {
  frc::sim::ElevatorSim sim =
      frc::sim::ElevatorSim::Create(frc::DCMotor::Vex775Pro(4), 100, 4_kg,
                                    0.5_in, 0_m, 10_m, false, 0_m, 9.8_mps_sq);
