...
TEST(DriverStationTest, AutonomousMode) {
  ...
  EXPECT_EQ(DriverStation::GetRobotMode(), DSControlWord::kAutonomous);
  EXPECT_TRUE(RobotState::IsAutonomous());
  EXPECT_EQ(RobotState::GetRobotMode(), DSControlWord::kAutonomous);
  EXPECT_TRUE(callback.WasTriggered());
...
TEST(DriverStationTest, Mode) {
  ...
  EnumCallback callback;
  auto cb =
      DriverStationSim::RegisterRobotModeCallback(callback.GetCallback(), false);
  ...
  EXPECT_EQ(DriverStation::GetRobotMode(), DSControlWord::kTest);
  EXPECT_TRUE(RobotState::IsTest());
  EXPECT_EQ(RobotState::GetRobotMode(), DSControlWord::kTest);
  EXPECT_TRUE(callback.WasTriggered());
