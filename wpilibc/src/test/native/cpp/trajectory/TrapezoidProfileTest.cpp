...
TEST(TrapezoidProfileTest, ReachesGoal) {
  frc::TrapezoidProfile<meter_t>::Constraints constraints{1.75_mps,
                                                          0.75_mps_sq};
  frc::TrapezoidProfile<meter_t>::State goal{3_m, 0_mps};
  frc::TrapezoidProfile<meter_t>::State state;

  for (int i = 0; i < 450; ++i) {
    frc::TrapezoidProfile<meter_t> profile{constraints, goal, state};
...
TEST(TrapezoidProfileTest, PosContinousUnderVelChange) {
  frc::TrapezoidProfile<meter_t>::Constraints constraints{1.75_mps,
                                                          0.75_mps_sq};
  frc::TrapezoidProfile<meter_t>::State goal{12_m, 0_mps};

  frc::TrapezoidProfile<meter_t> profile{constraints, goal};
  ...
  for (int i = 0; i < 1600; ++i) {
    ...
    profile = frc::TrapezoidProfile<meter_t>{constraints, goal, state};
    state = profile.Calculate(kDt);
...
TEST(TrapezoidProfileTest, Backwards) {
  frc::TrapezoidProfile<meter_t>::Constraints constraints{0.75_mps,
                                                          0.75_mps_sq};
  frc::TrapezoidProfile<meter_t>::State goal{-2_m, 0_mps};
  frc::TrapezoidProfile<meter_t>::State state;
...
TEST(TrapezoidProfileTest, TopSpeed) {
  frc::TrapezoidProfile<meter_t>::Constraints constraints{0.75_mps,
                                                          0.75_mps_sq};
  frc::TrapezoidProfile<meter_t>::State goal{4_m, 0_mps};
  frc::TrapezoidProfile<meter_t>::State state;
...
TEST(TrapezoidProfileTest, TimingToGoal) {
  ...
  for (int i = 0; i < 400; i++) {
    ...
    if (!reachedGoal && state == goal) {
      ...
      EXPECT_NEAR(predictedTimeLeft.to<double>(), i / 100.0, 0.25);
      reachedGoal = true;
    }
  }
}
...
TEST(TrapezoidProfileTest, TimingBeforeGoal) {
  ...
  for (int i = 0; i < 400; i++) {
    ...
    if (!reachedGoal &&
      ...
      EXPECT_NEAR(predictedTimeLeft.to<double>(), i / 100.0, 2e-2);
      reachedGoal = true;
    }
  }
}
