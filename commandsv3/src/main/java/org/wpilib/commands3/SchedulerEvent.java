...
public sealed interface SchedulerEvent {
  /**
   * The timestamp for when the event occurred. Measured in microseconds since some arbitrary
   * start time.
   *
   ...
   */
  record CompletedWithError(Command command,
                            Throwable error,
                            long timestampMicros) implements SchedulerEvent {
  }
   ...
   * afterward; when cancelled via {@link Scheduler#cancel(Command)} or
   * {@link Scheduler#cancelAll()}; or when they're running and interrupted by another command
  ...
  /**
   * An event marking when a command was interrupted by another. Typically followed by an
   * {@link Evicted} event.
   *
   ...
   */
  record Interrupted(Command command,
                     Command interrupter,
                     long timestampMicros) implements SchedulerEvent {
  }
}
