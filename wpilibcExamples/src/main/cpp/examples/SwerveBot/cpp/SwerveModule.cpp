...
SwerveModule::SwerveModule(const int driveMotorChannel,
  ...
  // resolution.
  driveEncoder.SetDistancePerPulse(2 * std::numbers::pi * WHEEL_RADIUS /
                                   ENCODER_RESOLUTION);
...
wpi::math::SwerveModulePosition SwerveModule::GetPosition() const {
  return {wpi::units::meters<>{driveEncoder.GetDistance()},
          wpi::units::radians<>{turningEncoder.GetDistance()}};
}

wpi::math::SwerveModuleVelocity SwerveModule::GetVelocity() const {
  return {wpi::units::meters_per_second<>{driveEncoder.GetRate()},
          wpi::units::radians<>{turningEncoder.GetDistance()}};
}
...
void SwerveModule::SetDesiredVelocity(
  ...
  const auto driveOutput =
      wpi::units::volts<>{drivePIDController.Calculate(
  ...
  const auto turnOutput =
      wpi::units::volts<>{turningPIDController.Calculate(
          wpi::units::radians<>{turningEncoder.GetDistance()},
