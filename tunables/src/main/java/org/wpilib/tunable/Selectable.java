...
public final class Selectable<V> implements ComplexTunable {
   ...
   * reset. Does not change the selected option; getSelected() will fall back to the default after
   * this is called if the selected option name no longer matches an option.
   *
   * @param name the name of the option
   */
  public void remove(String name) {
    requireNonNullParam(name, "name", "remove");
    if (m_map.containsKey(name)) {
      m_map.remove(name);
      m_options.set(m_map.keySet().toArray(new String[0]));
      if (name.equals(m_defaultChoice.get())) {
        m_defaultChoice.set("");
      }
    }
  }
   ...
   * getSelected() will return null after this is called until a new matching option or default is
   * added.
   */
  public void clear() {
    m_map.clear();
  ...
  public V getSelected() {
    ...
    if (selected != null && !selected.isEmpty() && m_map.containsKey(selected)) {
