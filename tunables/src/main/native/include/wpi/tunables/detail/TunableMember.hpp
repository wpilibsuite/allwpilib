 ...
 public:
  ...
  virtual void ForEachStructSchema(
      wpi::util::function_ref<void(std::string_view typeName,
          ...
          fn) const = 0;
  virtual bool UnpackStruct(ComplexTunable* obj,
...
class TunableMemberStruct : public detail::TunableMemberStructBase {
 public:
  ...
  void Set(ComplexTunable* obj, T value) {
    m_ptr.Get(obj) = std::move(value);
    SetTunableChanged();
  }

  T& Mutate(ComplexTunable* obj) {
    SetTunableChanged();
    return m_ptr.Get(obj);
  }
 ...
 protected:
  ...
  bool UnpackStruct(ComplexTunable* obj,
                    std::span<const uint8_t> data) override {
    if (data.size() != GetStructSize(obj)) {
      return false;
    }
    std::apply(
        [&](const I&... info) {
...
class TunableMemberStructVector : public detail::TunableMemberStructBase {
 public:
  ...
  std::vector<T>& Mutate(ComplexTunable* obj) {
    SetTunableChanged();
    return m_ptr.Get(obj);
  }
 ...
 protected:
  std::string GetStructTypeName() const override {
    return std::apply(
        [](const I&... info) {
          return std::string{
              wpi::util::MakeStructArrayTypeName<T, std::dynamic_extent>(
                  info...)};
        },
        m_info);
  }
  ...
  bool UnpackStruct(ComplexTunable* obj,
    ...
    bool unpacked = std::apply(
        [&](const I&... info) {
          size_t size = wpi::util::GetStructSize<T>(info...);
          if (size == 0 || data.size() % size != 0) {
            return false;
          }
          value.clear();
    ...
    std::apply(
        [&](const I&... info) {
          ...
          for (auto&& val : m_ptr.Get(obj)) {
            ...
            wpi::util::PackStruct(buf.first(size), val, info...);
...
class TunableMemberProtobuf : public detail::TunableMemberProtobufBase {
 ...
 protected:
  ...
  bool UnpackProtobuf(ComplexTunable* obj,
                      std::span<const uint8_t> data) override {
    auto value = m_message.Unpack(data);
    if (!value) {
      return false;
    }
    m_ptr.Get(obj) = std::move(*value);
