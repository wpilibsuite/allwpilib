...
## Prerequisites
- Install [Bazelisk](https://github.com/bazelbuild/bazelisk/releases) and add it to your bath. Bazelisk is a wrapper that will download the correct version of bazel specified in the repository. Note: You can alias/rename the binary to `bazel` if you want to keep the familiar `bazel build` vs `bazelisk build` syntax.
...
- `bazel test //ntcore:java-test` - Runs only the java test target in the ntcore folder
- `bazel coverage //wpiutil/...` - (*Nix only) - Runs a code coverage report for both C++ and Java on all the targets under wpiutil
- `bazel run //wpilibjExamples:gearsbot-example` - Runs the java gearsbot example. Examples can be run with HALSIM extensions enabled, which are specified in wpilib[c/j]Examples/BUILD.bazel
