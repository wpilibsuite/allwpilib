...
public class HatchMechanism implements Mechanism, TelemetryLoggable {
  ...
  public Command grabHatchCommand() {
    ...
    return this.run(coroutine -> {
      // Set the solenoid and then park the command, owning the mechanism until a different command
      // is scheduled and interrupts it. This makes command's telemetry match what the robot is
      // physically doing.
