...
public class ElevatorPIDF implements AutoCloseable {
  ...
  private double kEncoderDistPerPulse = 2.0 * Math.PI * 0.0508 / 4096.0; // 2" drum
  private DCMotor m_elevatorMotor;
  private Encoder m_encoder;
  private PWMSparkMax m_motor;

  // Control Hardware: WPILib controllers for position control with motion profiling
  ...
  public ElevatorPIDF() {
    ...
    // Set up profiled PID controller for position control with motion profiling
   ...
   * 2. Calculate the profiled PID + feedforward control output
  ...
  public void update() {
    ...
    // Step 2: Calculate
    m_controller.setGoal(m_desiredPosition);
    double pidOutput = m_controller.calculate(m_actualPosition);
