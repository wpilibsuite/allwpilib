...
public class Robot extends TimedRobot {
  ...
  public void robotInit() {
    ...
    Telemetry.publishNode("MyTab/PDP", m_pdp);
  }
  ...
  public void robotPeriodic() {
    ...
    Telemetry.putDouble("Current Channel 7", current7);

    // Get the voltage going into the PDP, in Volts.
    // The PDP returns the voltage in increments of 0.05 Volts.
    double voltage = m_pdp.getVoltage();
    Telemetry.putDouble("Voltage", voltage);

    // Retrieves the temperature of the PDP, in degrees Celsius.
    double temperatureCelsius = m_pdp.getTemperature();
    Telemetry.putDouble("Temperature", temperatureCelsius);

    // Get the total current of all channels.
    double totalCurrent = m_pdp.getTotalCurrent();
    Telemetry.putDouble("Total Current", totalCurrent);

    // Get the total power of all channels.
    // Power is the bus voltage multiplied by the current with the units Watts.
    double totalPower = m_pdp.getTotalPower();
    Telemetry.putDouble("Total Power", totalPower);

    // Get the total energy of all channels.
    // Energy is the power summed over time with units Joules.
    double totalEnergy = m_pdp.getTotalEnergy();
    Telemetry.putDouble("Total Energy", totalEnergy);
  }
}
