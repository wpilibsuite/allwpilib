...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    dpadUp.whenActive(new SetElevatorSetpoint(0.25, m_elevator));
    dpadDown.whenActive(new SetElevatorSetpoint(0.0, m_elevator));
    dpadRight.whenActive(new CloseClaw(m_claw));
    dpadLeft.whenActive(new OpenClaw(m_claw));

    r1.whenActive(new PrepareToPickup(m_claw, m_wrist, m_elevator));
    r2.whenActive(new Pickup(m_claw, m_wrist, m_elevator));
    l1.whenActive(new Place(m_claw, m_wrist, m_elevator));
    l2.whenActive(new Autonomous(m_drivetrain, m_claw, m_wrist, m_elevator));
  }
